...
import route_product_local_framing as route
...
def campaign(tmp_path,monkeypatch):
    ...
    names=[f'module_{n}.v' for n in range(43)]
    ...
    monkeypatch.setattr(route.local_framing,'witness',lambda text,is_aux: {'checks':10000,'publications':18,'local_rejections':6 if is_aux else 0,'auxiliary':is_aux})
    ...
    for p,is_aux in [(actual,False),(aux,True)]:
        ...
        save(p/'local_framing_outcome.json',dict(passed=True,product_local_framing=route.local_framing.witness(log,is_aux)))
...
@pytest.mark.parametrize('folder',[0,2])
@pytest.mark.parametrize('mutation',['missing','failed','changed_count','wrong_campaign'])
def test_local_framing_receipt_required(campaign,folder,mutation):
    path=campaign[folder]/'local_framing_outcome.json'
    value=json.loads(path.read_text())
    if mutation=='missing':path.unlink()
    else:
        if mutation=='failed':value['passed']=False
        elif mutation=='changed_count':value['product_local_framing']['checks']+=1
        else:value['product_local_framing']['auxiliary']=not value['product_local_framing']['auxiliary']
        save(path,value)
    with pytest.raises((ValueError,FileNotFoundError)):route.evidence(*campaign)
