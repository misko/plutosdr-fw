...
def test_exact_97_plus_19_source_closure(prepared):
    path,sha=prepared;result=b.verify(path,sha,live=True)
    assert result['sources']==116 and result['files']==119
    manifest=json.loads((path/'manifest.json').read_text())
    assert len(manifest['vectors'])==8 and len(manifest['compiled'])==26
    assert b.QUALIFIED97=='b52ba499ffbec7b4596afaba5b180d8145381ea56a4af60ab4cce30c79631908'
    assert set(b.qualified.source_names())<set(b.source_names())
    assert sum('retained_output_summary_candidate/' in x for x in manifest['compiled'])==4
    assert not any('scripted' in x for x in manifest['compiled'])
    assert c.inverse_bench((path/'bench.sv').read_text())==c.qualified.bench()
...
def test_mutated_bundle_never_admitted(prepared,tmp_path,mutation):
    ...
    if mutation=='sha':sha='0'*64
        ...
        if mutation=='source_join':
            ...
            name={'flag':'bench.sv','profile':'profile.tcl','runtime':'source_snapshot/hdl/library/starlink_pss_acquisition/retained_output_summary_candidate/starlink_pss_core_job_cutover.v'}[mutation]
            p=copy/name
            if mutation=='flag':p.write_text(p.read_text().replace('INPUT_OFFER_FAULT_SUMMARY=1','INPUT_OFFER_FAULT_SUMMARY=0',1))
...
def test_frozen_base_python_cli_from_root(prepared):
    ...
    run=subprocess.run([str(BASE_PYTHON),'-B',str(path/'source_snapshot/tools/prepare_starlink_retained_summary_actual.py'),
