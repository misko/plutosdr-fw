...
def test_rc22_owner_route_maps_branch_manifest_package_and_version() -> None:
    ...
    assert "Require the exact protected RC22 candidate identity" in workflow
...
def test_rc22_docs_preserve_truthful_rc21_hardware_failure() -> None:
    ...
    for source in (RELEASING, PLAN, KALMAN):
        ...
        assert "The active candidate is RC22" in text or "forward-only RC22" in text


def test_rc22_evidence_identity_and_attestation_policy_are_exact() -> None:
    evidence = EVIDENCE.read_text(encoding="utf-8")
    workflow = WORKFLOW.read_text(encoding="utf-8")
