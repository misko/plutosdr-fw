...
def test_integrated_counter_utc_source_matches_current_gitlinks() -> None:
    manifest = dict(
        line.split(": ", 1)
...
def test_integrated_counter_utc_identity_and_dual_rx_gate() -> None:
    ...
    assert "    ./scripts/check_source_graph.sh manifests/counter-utc-v054-source.yaml" in offline
    assert (
