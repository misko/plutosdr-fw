...
def test_rc23_owner_route_maps_branch_manifest_package_and_version() -> None:
    ...
    assert "Require the exact protected RC23 candidate identity" in workflow
...
def test_rc23_docs_preserve_truthful_rc22_hardware_results() -> None:
    ...
    for source in (RELEASING, PLAN, KALMAN):
        ...
        assert "The active candidate is RC23" in text or "forward-only RC23" in text


def test_rc23_evidence_identity_and_attestation_policy_are_exact() -> None:
    evidence = EVIDENCE.read_text(encoding="utf-8")
    workflow = WORKFLOW.read_text(encoding="utf-8")
