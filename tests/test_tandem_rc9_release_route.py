...
def test_rc9_owner_only_route_maps_ref_manifest_and_package_together() -> None:
    ...
    assert "Require the exact protected RC9 candidate identity" in workflow
...
def test_kalman_handoff_matches_the_rc9_bundle_checksum_contract() -> None:
    ...
    assert "The RC9 workflow has no separate attestation job." in runner
...
def test_rc9_release_docs_bind_exact_stage_locks_and_full_final_campaign() -> None:
    ...
    assert "The active candidate is RC9" in releasing
