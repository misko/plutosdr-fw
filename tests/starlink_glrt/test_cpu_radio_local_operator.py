...
def test_cycle_has_reviewed_distinct_los_and_explicit_worst_case(operator):
    ...
    assert operator.maximum_source_seconds(2,operator.WAIT40X2_CONFIRM_100_PROFILE) == pytest.approx(1395.130368)
    assert operator.maximum_source_seconds(4,operator.WAIT20X2_CONFIRM_100_PROFILE) == pytest.approx(1395.130368)
...
@pytest.mark.parametrize('profile',["sparse10-after-scout16","sparse30-after-scout16","sparse100-after-scout16",
    ...
    "sparse100-wait40x2-after-scout1","sparse100-wait40x2-confirm2-after-scout1",
...
def test_continuity_parent_retains_round_segment_and_sample_accounting(operator):
    ...
    confirm=continuity_parent(operator,rounds=2,segments=1,visits=5,result=1,
        scope="bounded_arm_scout_wait100_followup",
        rf_sample_limit=4*operator.SCOUT_SAMPLES+operator.FOLLOWUP_RF_SAMPLES,
        activity_selection="strongest_one_attempt_scan_power50_consecutive2_wait40x2_track100")
    assert operator.decode_parent(confirm,operator.RATE,2,1,
        operator.WAIT40X2_CONFIRM_100_PROFILE)["segments_started"]==1
    confirm20=continuity_parent(operator,rounds=2,segments=1,visits=9,result=1,
        scope="bounded_arm_scout_wait100_followup",
        rf_sample_limit=8*operator.SCOUT_SAMPLES+operator.FOLLOWUP_RF_SAMPLES,
        activity_selection="strongest_one_attempt_scan_power50_consecutive2_wait20x2_track100")
    assert operator.decode_parent(confirm20,operator.RATE,4,1,
        operator.WAIT20X2_CONFIRM_100_PROFILE)["segments_started"]==1
    for damaged in (
