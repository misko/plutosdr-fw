...
def check_bank(tmp_path, values, reset):
    ...
    cycles = [(1,0,1,0,phases-1)] + [(1,0,0,0,n % phases) for n in range(15)]
    cycles += [(0,0,0,0,0) if reset else (1,1,0,0,0), (1,0,0,0,0)]
    selected = [phase % phases for phase in (3, 1, 2, 0)]
    for phase in selected:
        cycles += [(1,0,1,0,phase)]
        cycles += [(1,0,0,int(n % 9 == 0),n % phases) for n in range(9*count+30)]
