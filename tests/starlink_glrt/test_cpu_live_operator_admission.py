...
@pytest.mark.parametrize('blocks,spacing,budget,valid',[
    (1536,3,64,True),(45000,3,64,True),(1536,3,80,True),(45000,3,80,True),
    (1536,9,64,False),(4096,3,64,False),(45000,9,64,False),(1536,9,80,False),
...
def test_expanded_scan_is_admitted_only_in_short_selected_profile(storage_operator,blocks,spacing,budget,valid):
    if valid:
        suffix='scan64' if budget==64 else 'scan80-local2'
        expected=f'1536-selected-observer3-{suffix}' if blocks==1536 else f'45000-selected-observer3-{suffix}'
