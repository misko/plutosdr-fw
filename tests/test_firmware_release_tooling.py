...
def test_direct_async_v4_retains_its_exact_historical_candidate_route() -> None:
    ...
    package = (ROOT / "scripts" / "ci" / "package_main_firmware.sh").read_text()
    checker = (ROOT / "scripts" / "check_tandem_release_offline.sh").read_text()
    branch = "refs/heads/codex/iq-direct-async-v4-exact-200m"
    manifest_name = "iq-direct-async-v4-source.yaml"
    manifest = (ROOT / "manifests" / manifest_name).read_text()
    ...
    assert current_worktree_checks == [
        "./scripts/check_source_graph.sh manifests/fixed-dwell-v057-source.yaml"
    ]
    for source in (builder, package, checker):
        assert manifest_name in source
    ...
    assert "libiio_0_25_source: 5cb2389719d46d12463daa0371d1fda19eb25fa7" in manifest
    assert "submodule_buildroot: 2e146948a52eaf7c7f675c5e6ac746eeff4aacac" in manifest
    assert "submodule_linux: 7176508dd84bde78c62d8790bbd17957fdda12d7" in manifest
    assert "versions_buildroot: iq-direct-async-v4-source/buildroot-v1" in manifest
