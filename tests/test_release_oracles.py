...
DDR_RING_V1_RC1_SOURCE_MANIFEST = (
    ROOT / "manifests" / "ddr-ring-v1-rc1-source.yaml"
)
DDR_RING_V1_RC2_SOURCE_MANIFEST = (
    ROOT / "manifests" / "ddr-ring-v1-rc2-source.yaml"
)
DDR_RING_PREFILL_V1_RC1_SOURCE_MANIFEST = (
    ROOT / "manifests" / "ddr-ring-prefill-v1-rc1-source.yaml"
)
DIRECT_ASYNC_RING_V1_RC1_SOURCE_MANIFEST = (
    ROOT / "manifests" / "iq-direct-async-ring-v1-rc1-source.yaml"
)
DIRECT_ASYNC_V2_SOURCE_MANIFEST = (
    ROOT / "manifests" / "iq-direct-async-v2-source.yaml"
)
...
def test_ddr_ring_v1_rc1_advances_only_libiio_and_buildroot() -> None:
    previous = _manifest_values(DDR_BURST_V2_RC3_SOURCE_MANIFEST)
    current = _manifest_values(DDR_RING_V1_RC1_SOURCE_MANIFEST)
    changed = {key for key, value in previous.items() if current[key] != value}
    ...
    assert current["libiio_0_25_source"] == (
        "739a250b92610184b12d773f6a367e549f0dfe29"
    )
    assert current["submodule_buildroot"] == (
        "879afd8facb69519ed2328b39d80d6905e416247"
    )
    assert "release_tag" not in current


def test_ddr_ring_v1_rc2_advances_only_libiio_and_buildroot() -> None:
    previous = _manifest_values(DDR_RING_V1_RC1_SOURCE_MANIFEST)
    current = _manifest_values(DDR_RING_V1_RC2_SOURCE_MANIFEST)
    changed = {key for key, value in previous.items() if current[key] != value}
    ...
    assert current["submodule_buildroot"] == (
        "afe53e01c2356125227bd58b5551ad9a6aae1121"
    )
    assert "release_tag" not in current


def test_ddr_ring_prefill_v1_rc1_advances_only_libiio_and_buildroot() -> None:
    previous = _manifest_values(DDR_RING_V1_RC2_SOURCE_MANIFEST)
    current = _manifest_values(DDR_RING_PREFILL_V1_RC1_SOURCE_MANIFEST)
    changed = {key for key, value in previous.items() if current[key] != value}
...
def test_direct_async_ring_v1_rc1_pins_the_qualified_source_graph() -> None:
    values = _manifest_values(DIRECT_ASYNC_RING_V1_RC1_SOURCE_MANIFEST)
...
def test_direct_async_v2_pins_the_candidate_source_graph() -> None:
    values = _manifest_values(DIRECT_ASYNC_V2_SOURCE_MANIFEST)
    ...
    assert values["libiio_0_25_ref"] == (
        "refs/tags/iq-direct-async-v2-source/libiio-v1"
    )
    assert values["libiio_0_25_archive_sha256"] == (
        "eb179eef9e4ee2accbd9e5d1fdc5e4a6d7f0a76675ac0669e8b128e88bb9933b"
    )
    assert values["submodule_buildroot"] == (
        "3e1dd15acf361cc06e202e9e59e907dd379a13c3"
    )
    assert values["versions_buildroot"] == (
        "iq-direct-async-v2-source/buildroot-v1"
    )


def test_direct_async_v3_pins_the_candidate_source_graph() -> None:
    values = _manifest_values(DIRECT_ASYNC_V3_SOURCE_MANIFEST)
    ...
    assert values["libiio_0_25_ref"] == (
        "refs/tags/iq-direct-async-v3-source/libiio-v1"
    )
    assert values["libiio_0_25_archive_sha256"] == (
        "66ccc7230ebe75c477c4dfc147aa86289c3f896c0a0d6b3b6c964e152d89c266"
    )
    assert values["submodule_buildroot"] == (
        "1c337a0b8d8126c9d1ed785607bc5ea52e7fed22"
    )
    assert values["submodule_buildroot_ref"] == (
        "refs/tags/iq-direct-async-v3-source/buildroot-v1"
    )
    assert values["versions_buildroot"] == (
