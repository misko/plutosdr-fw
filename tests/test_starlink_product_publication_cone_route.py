...
import route_product_publication_cone as route
...
def campaign(tmp_path,monkeypatch):
    ...
    names=[f'module_{n}.v' for n in range(40)]
    ...
    monkeypatch.setattr(route.publication,'witness',lambda text,is_aux: {'checks':10000,'enabled':18,'faults':8 if is_aux else 0,'auxiliary':is_aux})
    ...
    for p,is_aux in [(actual,False),(aux,True)]:
        ...
        save(p/'publication_cone_outcome.json',dict(passed=True,product_publication_cone=route.publication.witness(log,is_aux)))
...
def test_publication_cone_receipt_required(campaign,folder,mutation):
    path=campaign[folder]/'publication_cone_outcome.json'
    value=json.loads(path.read_text())
    if mutation=='missing':path.unlink()
    else:
        if mutation=='failed':value['passed']=False
        elif mutation=='changed_count':value['product_publication_cone']['checks']+=1
        else:value['product_publication_cone']['auxiliary']=not value['product_publication_cone']['auxiliary']
        save(path,value)
    with pytest.raises((ValueError,FileNotFoundError)):route.evidence(*campaign)
