...
def test_rc15_owner_route_maps_branch_manifest_package_and_version_together() -> None:
    ...
    assert "Require the exact protected RC15 candidate identity" in workflow
...
def test_rc15_pins_the_corrected_pushed_utility() -> None:
    expected = "5ab8361211e747387c5dfa854f5ae65a6a4dac87"
    for path in (WRAPPER, BINDING, RC15_MANIFEST):
...
def test_rc15_evidence_identity_and_docs_are_active() -> None:
    evidence = EVIDENCE.read_text(encoding="utf-8")
    sources = tuple(
        path.read_text(encoding="utf-8") for path in (RELEASING, NOTES, PLAN, KALMAN)
    )

    assert "v0.41-plutoplus-spf-tandem-agc-v8-rc15" in evidence
    assert "refs/tags/tandem-agc-v8-rc15-source/firmware-v1" in evidence
    for source in sources:
        assert "RC15" in source
        assert "5ab8361211e747387c5dfa854f5ae65a6a4dac87" in source
