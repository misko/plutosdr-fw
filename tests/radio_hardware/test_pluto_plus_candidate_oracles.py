...
VERSION = "v0.41-plutoplus-spf-tandem-agc-v8-rc28"
INDEX_SHA = "a" * 64
DFU_SHA = "b" * 64
FIT_SHA = "c" * 64
INDEX_BYTES = 9_890
...
def _artifact_index() -> dict[str, Any]:
    return {
        ...
        "source": {
            "commit": "1" * 40,
            "manifest_path": "source/tandem-agc-v8-rc28-source.yaml",
...
def _candidate() -> dict[str, Any]:
    return {
        ...
        "artifact_index": {
            "path": "/private/rc20/candidate-index.json",
            "bytes": INDEX_BYTES,
            "sha256": INDEX_SHA,
        },
        "dfu": {
            "path": "/private/rc20/artifact/firmware.dfu",
            "bytes": 8_388_624,
            "sha256": DFU_SHA,
        },
...
def _operation(candidate: object, inventory: object) -> dict[str, Any]:
    return {
        ...
        "candidate_plan": _identity(
            f"/private/rc20/hardware/deploy/{SERIAL}/release-candidate-plan.json",
            candidate,
        ),
        "usb_inventory": _identity(
            f"/private/rc20/hardware/deploy/{SERIAL}/usb-inventory.json", inventory
        ),
        "target": _target(),
        "expected_current_firmware": "v0.41-plutoplus-spf-tandem-agc-v8-rc12",
        "ssh_host": "192.168.2.1",
        "receipt_path": f"/private/rc20/hardware/deploy/{SERIAL}/ram-boot-receipt.json",
...
def _receipt(candidate: object, operation: object) -> dict[str, Any]:
    return {
        ...
        "operation_plan": _identity(
            f"/private/rc20/hardware/deploy/{SERIAL}/operation-plan.json", operation
        ),
        "candidate_plan": _identity(
            f"/private/rc20/hardware/deploy/{SERIAL}/release-candidate-plan.json",
            candidate,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda c, _i, _o, _r: c["expected_runtime"].update(
            metadata_abi="frame-metadata-v5"
        ),
...
def test_candidate_plan_rejects_buffer_abi_as_release_frame_schema() -> None:
    candidate = _candidate()
    artifact = _artifact_index()
    artifact["release"]["metadata_abi"] = PLUTO_IIO_BUFFER_METADATA_ABI

    with pytest.raises(CandidateBindingError, match="expected runtime"):
        validate_release_candidate_plan(
            candidate,
