"""Route requires matching actual and synthesis evidence for split preflight."""
import json
...
ARTIFACTS=Path('/dev/shm/starlink-split-preflight.2BQ3zjL5')
...
def test_complete_matching_actual_evidence():
    ...
    experiment.verify_split_preflight_configuration(prepared)
    main=experiment.audit_split_preflight(ARTIFACTS/'sim-v1')
    aux=route.verify_ack_auxiliary(ARTIFACTS/'ack-v1',PIN,prepared)
    assert main['numerical_rows']==64512
    for result in [main,aux]:assert result['split_preflight']['source']>=20 and result['split_preflight']['product']>=20

@pytest.mark.parametrize('mutation',['missing','duplicate','empty_source','empty_product','current_exact','latency_unchanged'])
def test_incomplete_or_changed_evidence_rejected(tmp_path,mutation):
    text=(ARTIFACTS/'ack-v1'/SIM/'simulate.log').read_text()
    line=next(x for x in text.splitlines(True) if x.startswith('STAGED_SPLIT_PREFLIGHT_PASS '))
    ...
    with pytest.raises(ValueError):experiment.audit_split_preflight(tmp_path,auxiliary=True)
...
def test_missing_compiled_campaign_blocks_route_before_output(tmp_path):
    ...
    outcome=json.loads((ARTIFACTS/'sim-v1/outcome.json').read_text());del outcome['audit']['split_preflight']
    (main/'outcome.json').write_text(json.dumps(outcome))
    with pytest.raises(ValueError,match='compiled split preflight'):
...
def test_compilation_mismatch_rejected(tmp_path,mutation):
    for name in ['tb_fft_staged_output.sv','staged_fft_experiment.tcl']:
        ...
        if mutation=='bench_disabled':text=text.replace('.SPLIT_PREFLIGHT_IDENTITY(1)','.SPLIT_PREFLIGHT_IDENTITY(0)',1)
        else:text=text.replace(' SPLIT_PREFLIGHT_IDENTITY=1',' SPLIT_PREFLIGHT_IDENTITY=0',1)
        (tmp_path/name).write_text(text)
    with pytest.raises(ValueError,match='matching enabled split preflight'):experiment.verify_split_preflight_configuration(tmp_path)
