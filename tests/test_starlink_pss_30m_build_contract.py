...
MANIFEST_NAME = "starlink-pss-multirate-rx-only-dnm-v8-source.yaml"
...
def test_v8_manifest_is_nonpersistent_30_msps_acquisition_only() -> None:
    ...
    for contract in (
        ...
        "source_change: qualify-30m-upper-edge-ddc-acquisition",
...
def test_v8_build_and_package_routes_are_exactly_30_msps() -> None:
    ...
    for source in (builder, packager):
        assert "v8 acquisition-only qualification is gated to 30 MS/s" in source
    ...
    assert (
        ...
        '${STARLINK_PSS_RATE_MSPS}m-rx-only-dnm-v8"'
    ) in packager


def test_candidate_planner_admits_but_does_not_default_to_v8() -> None:
    planner = _read("scripts/starlink_pss_multirate_candidate_plan.py")

    assert f'"{MANIFEST_NAME}": "v8"' in planner
    assert '{"v2", "v3", "v4", "v5", "v6", "v7", "v8"}' in planner
    assert (
