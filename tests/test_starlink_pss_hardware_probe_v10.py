...
import scripts.starlink_pss_hardware_probe_v10 as probe
...
def test_v10_profile_attestation_accepts_exact_v8_candidate(tmp_path: Path) -> None:
    probe._require_acquisition_only_profile(_profile_handoff(tmp_path))


def test_v10_profile_attestation_rejects_wrong_rate(tmp_path: Path) -> None:
    ...
    with pytest.raises(probe.ProbeError, match="30 MS/s v8 profile"):
        probe._require_acquisition_only_profile(handoff)


def test_v10_contract_scopes_identity_revision_and_loader(monkeypatch) -> None:
...
def test_v10_wraps_dependency_safe_probe_under_the_v8_contract(monkeypatch) -> None:
    ...
    assert observed == [("ad9361-1r1t", ("v2", "v3", "v4", "v5", "v6", "v8"))]


def test_v10_probe_manifest_is_offline_dnm_and_byte_exact() -> None:
    root = Path(__file__).resolve().parents[1]
    manifest = root / "manifests/starlink-pss-hardware-probe-dnm-v10-source.yaml"
    values = {
    ...
    assert values["schema_version"] == "10"
    ...
    assert values["supported_candidate_revision"] == "v8"
