...
from .iq_bank_owned_evidence import digest, parse_run
...
def receipt(tmp_path: Path, transcript: str, mode: str = "numeric") -> Path:
    ...
    source = directory / "frozen_sources/design.v"
    source.parent.mkdir()
    ...
    (directory / "scope.txt").write_text(
        f"fast_mhz=175 slow_mhz=100 source_msps=15\n{digest(source)}  {source}\n"
...
@pytest.mark.parametrize("bad", [
    ...
    NUMERIC.replace("autonomous_gap_index_recovery=1", "autonomous_gap_index_recovery=0"),
...
def test_source_must_match_pre_run_hash(tmp_path):
    ...
    (directory / "frozen_sources/design.v").write_text("changed after measurement\n")
...
def test_capacity_has_full_continuous_inventory(tmp_path):
    result = parse_run(receipt(tmp_path, CAPACITY, "capacity"), "capacity", 175, "bursty-stalled")
