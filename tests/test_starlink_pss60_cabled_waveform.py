...
from tests.starlink_oracle import projected_pss, quantize_q15, x4_ddc_ci16
from tools.generate_starlink_pss60_cabled_waveform import (
    FRAME_SAMPLES,
    SAMPLE_RATE_HZ,
    TEMPLATE_PHASE,
    TEMPLATE_SAMPLES,
...
def test_60m_cabled_waveform_is_exact_and_ddc_ready(tmp_path) -> None:
    ...
    ddc = x4_ddc_ci16(waveform, first_input_index=0, edge="upper")
    assert ddc.discontinuities == 0
    assert ddc.saturation_events == 0
    assert evidence["nominal_detector_peak_phase_bin_canonical"] == 4096
    assert evidence["ddc"]["stage_60_to_30_samples"] == 39_993
    assert evidence["ddc"]["canonical_15m_samples"] == 19_989
    assert evidence["waveform"]["bytes"] == FRAME_SAMPLES * 4


def test_60m_cabled_waveform_generation_is_absent_only(tmp_path) -> None:
    generate(tmp_path)
