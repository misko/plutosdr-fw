...
def test_kalman_handoff_matches_the_rc7_bundle_checksum_contract() -> None:
    runner = KALMAN_RUNNER.read_text(encoding="utf-8")

    assert "The RC7 workflow has no separate attestation job." in runner
...
def test_rc7_release_docs_bind_exact_stage_locks_and_full_final_campaign() -> None:
    ...
    assert "The active candidate is RC7" in releasing
