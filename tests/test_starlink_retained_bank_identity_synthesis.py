...
ASSETS = ROOT/'tools/retained_bank_identity_synthesis'
...
p = module('summary_prepare_policy', ROOT/'tools/prepare_starlink_retained_bank_identity_synthesis.py')
...
def test_whole_reference_and_three_runtime_pins():
    pins=json.loads((ASSETS/'source_pins.json').read_text())
    assert len(pins['runtime'])==16
    assert sum('/baseline/' in n for n in pins['runtime'])==8
    four={n:sha for n,sha in pins['runtime'].items() if '/retained_output_bank_identity_candidate/' in n}
    assert len(four)==3
...
def mock_actual(tmp_path,monkeypatch):
    ...
    cli=put(qualified/'source_snapshot/tools/prepare_starlink_retained_bank_identity_actual.py','# MOCK_ONLY_CLI_NOT_EXECUTED\n')
...
def test_mock_only_actual_rejection(mock_actual,change):
    ...
    elif change=='cli_changed':put(qualified/'source_snapshot/tools/prepare_starlink_retained_bank_identity_actual.py','changed')
...
def mock_tcl_bundle(tmp_path):
    ...
    text=(ASSETS/'synthesize_retained_bank_identity.tcl').read_text()
    text=text.replace(d.ACTUAL_MANIFEST,'a'*64)
    put(root/'synthesize_retained_bank_identity.tcl',text)
    for name in ('clocks.xdc','threads.tcl','inspect_bank_identity.tcl'):
        shutil.copyfile(ASSETS/name,root/name)
    put(root/'qualified/source_snapshot/tools/prepare_starlink_retained_bank_identity_actual.py',
...
source {{{bundle/'synthesize_retained_bank_identity.tcl'}}}
'''
    ...
    assert 'RETAINED_BANK_IDENTITY_SYNTHESIS_RECORDED' not in run.stdout
...
def test_optimizer_cannot_disable_admission(tmp_path):
    run=subprocess.run([p.PYTHON,'-O','-B',str(ROOT/'tools/prepare_starlink_retained_bank_identity_synthesis.py'),
...
def test_mock_tcl_admission_never_crosses_vendor_boundary(mock_tcl_bundle,tmp_path,change):
    ...
    if change=='digest':sha='0'*64
        ...
        put(bundle/'synthesize_retained_bank_identity.tcl',
            (ASSETS/'synthesize_retained_bank_identity.tcl').read_text().replace(d.ACTUAL_MANIFEST,d.UNBOUND))
...
def test_original_actual_is_not_new_candidate_authority(tmp_path, monkeypatch):
    assert g.ACCEPTED_ACTUAL is None and g.EXPECTED == d.UNBOUND
    def forbidden(*args, **kwargs):
        pytest.fail('unbound candidate touched actual verification or copying')
    monkeypatch.setattr(p.subprocess, 'check_output', forbidden)
    monkeypatch.setattr(p.shutil, 'copytree', forbidden)
    old = Path('/home/mouse9911/gits/starlink-build-recovery-20260910.vHzUVnBz')
    with pytest.raises(ValueError, match='UNBOUND'):
        p.prepare(old/'retained-summary-actual-prelaunch-v1',
                  old/'retained-summary-actual-parent.XYAbt6Np/run',tmp_path/'no_bundle')
    for expected in (d.UNBOUND, d.OLD_MANIFEST, '0'*64, '', None):
        with pytest.raises(ValueError, match='UNBOUND'):
            g.load_binding(expected)
    assert not (tmp_path/'no_bundle').exists()


def test_exact_three_runtime_inverses_and_thirteen_unchanged():
    pins = json.loads((ASSETS/'source_pins.json').read_text())
    assert pins == d.pins() and pins['actual_execution_accepted'] is False
    previous = json.loads((ASSETS/'reference/summary_source_pins.json').read_text())['runtime']
    unchanged = {n:h for n,h in previous.items() if Path(n).name not in d.REPLACEMENTS}
    assert len(unchanged) == 13
    assert all(pins['runtime'][n] == h for n,h in unchanged.items())
    recipe = json.loads((ASSETS/'reference/retained_bank_identity_recipe.json').read_text())
    changes = json.loads((ASSETS/'reference/retained_bank_identity_inverse.json').read_text())
    assert digest(ASSETS/'reference/retained_bank_identity_inverse.json') == recipe['inverse_sha256']
    assert recipe['candidate_sources'] == d.REPLACEMENTS
    for name, expected in d.REPLACEMENTS.items():
        path = ASSETS/'reference/runtime'/name
        assert digest(path) == expected
        text = path.read_text()
        for before, after in reversed(changes[name]):
            assert text.count(after) == 1
            text = text.replace(after,before,1)
        assert hashlib.sha256(text.encode()).hexdigest() == recipe['original_sources'][name]


@pytest.mark.parametrize('mutant',['bind_old','remove_failclosed','drop_runtime','erase_whole_result'])
def test_admission_source_inverse_rejects_policy_mutants(mutant):
    _,_,text,_ = d.recipe('admission')
    before, after = {
        'bind_old':('ACCEPTED_ACTUAL = None','ACCEPTED_ACTUAL = {}'),
        'remove_failclosed':('ACCEPTED_ACTUAL is not None','True'),
        'drop_runtime':('len(runtime) == 16','len(runtime) >= 0'),
        'erase_whole_result':("json.loads(result) == json.loads((actual/'results.json').read_text())",'True'),
    }[mutant]
    assert before in text
    with pytest.raises(ValueError,match='inverse'):
        d.inverse('admission',text.replace(before,after))


@pytest.mark.parametrize('replacement',['','BANK_LOCAL_IDENTITY_EQ=0',
    "BANK_LOCAL_IDENTITY_EQ=1'bx","BANK_LOCAL_IDENTITY_EQ=1'bz",'BANK_LOCAL_IDENTITY_EQ=1 EXTRA=1'])
def test_new_parameter_actual_readback_is_mandatory(tmp_path,replacement):
    text=(ASSETS/'synthesize_retained_bank_identity.tcl').read_text()
    fragment=text[text.index('  set generics [list '):text.index('  set_property STEPS.SYNTH_DESIGN.ARGS.FLATTEN_HIERARCHY')]
    script='''set kernel /MOCK_ONLY/kernel.mem
proc get_filesets args {return sources_1}
proc set_property {n value target} {global actual;set actual $value}
proc get_property args {global actual;return [string map [list BANK_LOCAL_IDENTITY_EQ=1 {'''+replacement+'''}] $actual]}
'''+fragment
    result=tcl_run(tmp_path,script)
    assert result.returncode != 0 and 'bank-local-identity generics' in result.stderr


def test_complete_physical_source_closure_is_copied(mock_actual,monkeypatch):
    root,qualified,_,actual,_=mock_actual
    monkeypatch.setattr(p.subprocess,'check_output',lambda command,**kwargs:
        (actual/'results.json').read_text() if command[3]=='results' else '{"MOCK_ONLY":true}')
    out=root/'MOCK_ONLY_complete_closure'
    p.prepare(qualified,actual,out)
    for name,source in p.ASSETS.items():
        assert (out/name).read_bytes() == (ROOT/source).read_bytes()
    assert 'inspect_bank_identity.tcl' in p.ASSETS
    assert all(name in p.ASSETS for name in
        ('reference/synthesis_owner.py','reference/route_owner.py',
         'reference/retained_bank_identity_inverse.json','reference/retained_bank_identity_recipe.json'))


def mock_mapped_nets():
    # Independent spelling fixture for mapped-name queries, NOT a mapped FPGA.
    stems = {
        'preflight_source':'retained.island/bank_local_preflight.banks[0].balanced',
        'preflight_product':'retained.island/bank_local_preflight.banks[1].balanced',
        'preflight_fallback':'retained.island/balanced_preflight.comparisons[0]',
        'preflight_expected':'retained.island/balanced_preflight.comparisons[1]',
        'guard_source':'retained.island/input_guard/bank_local_identity.banks[0].balanced',
        'guard_product':'retained.island/input_guard/bank_local_identity.banks[1].balanced',
        'guard_fallback':'retained.island/input_guard/balanced_identity',
    }
    return {label:[f'{stem}.{kind}[{index}]' for kind,width in
        [('leaf_equal',24),('group_equal',4)] for index in range(width)]
        for label,stem in stems.items()}


def inspection_run(tmp_path,nets,again=False):
    out=tmp_path/'MOCK_ONLY_MAPPED';out.mkdir(exist_ok=True)
    # Actual Vivado intrinsic calls are replaced only here; the observer itself
    # is unmodified. All names, cells, endpoints and timing files are synthetic.
    text='set MOCK_NETS {'+' '.join('{'+n+'}' for n in nets)+'}\n'+'''
proc get_filesets args {return MOCK_ONLY_sources}
proc get_nets args {global MOCK_NETS;return $MOCK_NETS}
proc get_property {property object} {
  if {$property eq "NAME"} {return $object}
  if {$property eq "KEEP"} {return true}
  if {$property eq "REF_NAME"} {return MOCK_ONLY_LUT6}
  return MOCK_ONLY_BANK_LOCAL_IDENTITY_EQ_1
}
proc get_pins args {return MOCK_ONLY_driver/O}
proc get_cells args {return MOCK_ONLY_driver}
proc all_fanin args {
  if {[lsearch -exact $args -startpoints_only]>=0} {
    return {MOCK_ONLY_metadata_reg/Q MOCK_ONLY_descriptor_reg/Q}
  }
  return MOCK_ONLY_combinational_cell
}
proc all_fanout args {return MOCK_ONLY_endpoint/D}
proc report_timing args {
  set target [lindex $args [expr {[lsearch -exact $args -file]+1}]]
  set f [open $target {WRONLY CREAT EXCL}];puts $f MOCK_ONLY_NOT_TIMING;close $f
}
'''+f'source {{{ASSETS/"inspect_bank_identity.tcl"}}}\npss_bank_identity_inspect {{{out}}}\n'
    if again:text+=f'pss_bank_identity_inspect {{{out}}}\n'
    return out,tcl_run(tmp_path,text)


@pytest.mark.parametrize('change',['none','missing_preflight_product','missing_guard_fallback',
                                   'missing_preflight_fallback','duplicate_bank','wrong_last_leaf'])
def test_mapped_observer_records_both_banks_and_fallbacks(tmp_path,change):
    families=mock_mapped_nets()
    if change.startswith('missing_'):families.pop(change.removeprefix('missing_'))
    nets=[n for group in families.values() for n in group]
    if change=='duplicate_bank':nets+=['other.'+families['guard_product'][0]]
    if change=='wrong_last_leaf':nets=[n.replace('leaf_equal[23]','leaf_equal[24]') for n in nets]
    out,result=inspection_run(tmp_path,nets)
    assert result.returncode==0,result.stderr
    receipt=(out/'bank_identity_mapped_inventory.txt').read_text()
    assert 'factorization_verified false' in receipt
    assert f'all_expected_kept_names_observed {int(change=="none")}' in receipt
    if change=='none':
        assert 'observed_kept_objects 196' in receipt
        assert len(list(out.glob('bank_identity_*_max.rpt')))==7
        assert len(list(out.glob('bank_identity_*_min.rpt')))==7
        for label in families:
            assert f'match {label} leaf_equal 23 count 1' in receipt
        assert receipt.count('fanin_startpoints')==196
        assert 'MOCK_ONLY_metadata_reg/Q MOCK_ONLY_descriptor_reg/Q' in receipt


def test_mapped_observer_refuses_overwrite(tmp_path):
    out,result=inspection_run(tmp_path,[n for row in mock_mapped_nets().values() for n in row],True)
    assert result.returncode!=0 and 'file already exists' in result.stderr
    assert 'all_expected_kept_names_observed 1' in (out/'bank_identity_mapped_inventory.txt').read_text()


def test_mapped_observer_is_read_only_and_hook_is_source_bound():
    observer=(ASSETS/'inspect_bank_identity.tcl').read_text()
    forbidden=('set_property','create_clock','set_false_path','set_clock_groups','set_multicycle_path',
               'phys_opt_design','opt_design','place_design','route_design','launch_runs','open_checkpoint')
    assert not any(word in observer for word in forbidden)
    tcl=(ASSETS/'synthesize_retained_bank_identity.tcl').read_text()
    assert tcl.count('pss_bank_identity_inspect $output')==1
    assert tcl.index('open_run synth_1') < tcl.index('pss_bank_identity_inspect $output') < tcl.index('  close_project')
    for mutation in ('pss_bank_identity_inspect $output','source [file join $output inspect_bank_identity.tcl]'):
        with pytest.raises(ValueError,match='inverse'):
            d.inverse('tcl',tcl.replace(mutation,'# missing observer'))
