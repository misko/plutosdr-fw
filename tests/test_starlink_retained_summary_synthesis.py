...
p = module('summary_prepare_policy', ROOT/'tools/prepare_starlink_retained_summary_synthesis.py')
...
def mock_actual(tmp_path,monkeypatch):
    ...
    cli=put(qualified/'source_snapshot/tools/prepare_starlink_retained_summary_actual.py','# MOCK_ONLY_CLI_NOT_EXECUTED\n')
...
def test_mock_only_actual_rejection(mock_actual,change):
    ...
    elif change=='cli_changed':put(qualified/'source_snapshot/tools/prepare_starlink_retained_summary_actual.py','changed')
...
def mock_tcl_bundle(tmp_path):
    ...
    put(root/'qualified/source_snapshot/tools/prepare_starlink_retained_summary_actual.py',
...
source {{{bundle/'synthesize_retained_summary.tcl'}}}
'''
    ...
    assert 'RETAINED_SUMMARY_SYNTHESIS_RECORDED' not in run.stdout
...
def test_optimizer_cannot_disable_admission(tmp_path):
    run=subprocess.run([p.PYTHON,'-O','-B',str(ROOT/'tools/prepare_starlink_retained_summary_synthesis.py'),
