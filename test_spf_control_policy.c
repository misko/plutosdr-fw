...
static cmd_usb_start_rx_v1_t valid_v3_request(void)
{
	cmd_usb_start_rx_v1_t request = {
		.magic = SPF_GADGET_START_V3_MAGIC,
		.protocol_version = SPF_GADGET_PROTOCOL_V3,
		.request_bytes = sizeof(cmd_usb_start_rx_v1_t),
		.requested_features = SPF_META_REQUIRED_FEATURES_V3,
		.enabled_scan_mask = UINT32_C(0x0F),
		.samples_per_channel = SPF_GADGET_MAX_SAMPLES_PER_CHANNEL,
		.frame_count = 1,
		.reserved0 = UINT32_C(32768),
		.reserved1 = UINT32_C(16),
	};
	return request;
}
...
int main(void)
{
	...
	request = valid_v3_request();
	assert(spf_validate_start_rx_versioned(&request) == SPF_START_VALID);
	request.reserved0 = 0;
	assert(spf_validate_start_rx_versioned(&request) ==
		SPF_START_INVALID_OBSERVATION_INTERVAL);
	request = valid_v3_request();
	request.reserved1 = SPF_MAX_GAIN_OBSERVATIONS + 1;
	assert(spf_validate_start_rx_versioned(&request) ==
