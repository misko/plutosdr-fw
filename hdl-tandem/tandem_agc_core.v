...
module tandem_agc_core #(
  ...
  always @(posedge l_clk) begin
    if (!l_resetn) begin
      ...
      // evt_seq is the next sequence presented to the FIFO. It advances on the
      // registered push handoff, after the FIFO has sampled the current value,
      // so the first emitted event is zero without a set-valued reset bank.
      evt_seq <= 32'd0;
