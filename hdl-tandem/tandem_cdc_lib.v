...
module tandem_async_fifo #(
  ...
  always @(posedge wr_clk) begin
    if (!wr_resetn) begin
      ...
      rgray_s2 <= rgray_s1;
      if (wr_en) begin
        // Saturate. A counter that wraps reports "almost none" after 256
        // drops, which is worse than silence: the frame would look healthy.
        if (full_r && wr_ovf != 8'hFF) wr_ovf <= wr_ovf + 8'd1;   // §7.5
