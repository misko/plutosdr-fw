...
output_dir="${1:-${repository_root}/build/starlink-pss15-iq-to-phase-map-xfft}"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$acquisition_dir/simulate_iq_to_phase_map_xfft.tcl" \
      -tclargs "$simulation_dir" "$vector_dir" 2>&1 | \
      tee iq_to_phase_map_xfft_transcript.log
)

transcript="$simulation_dir/iq_to_phase_map_xfft_transcript.log"
if grep -q 'IQ_TO_PHASE_MAP_XFFT_FAIL' "$transcript" ||
   ! grep -q 'IQ_TO_PHASE_MAP_XFFT_PASS .*exact_map_reads=447' "$transcript"; then
  printf '%s\n' 'The IQ-to-phase-map XFFT replay did not produce a clean PASS.' >&2
  exit 1
fi
