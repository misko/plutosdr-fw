...
def main():
    ...
    for name in (
        ...
        for cell in result["cells"]:
            ...
            for previous, current in pairwise(frames):
                assert current["first"] - previous["end"] == current["missing"]
            missing = sum(frame["missing"] for frame in frames)
            if name == "candidate-1r1t-loss":
                assert missing > 0 and len(frames) == 100
            else:
                assert missing == 0
            if result["counter_only"]:
                assert cell["allocated_kernel_buffers"] == 50
                assert all(
                    frame["end"] - frame["first"] == 1_000_000 for frame in frames
                )
    disconnect = json.loads((ROOT / "disconnect.json").read_text())
    assert all(
        disconnect[key] is True
        for key in (
            "killed_client_releases_and_recaptures",
            "timeout_releases_and_recaptures",
            "settings_restored",
            "timestamp_restored",
        )
    )
    ...
    assert all(
        ...
        for key in (
            ...
            "gain_write",
            "timestamp_write",
            "second_counter_owner",
        )
    )
    assert (
        lifecycle["normal_close_restores_timestamp"] and lifecycle["settings_restored"]
    )
    ...
    policy = IQ_DIRECT_ASYNC_V4_CANDIDATE_RAM_POLICY.model_copy(
        update={
            "profile_id": "issue-97-counter-rx-v1-release-local-persistent",
    ...
    profile = replace(
        ...
        allowed_before_firmwares=("v0.49-plutoplus-spf-counter-rx-v1-rc1",),
