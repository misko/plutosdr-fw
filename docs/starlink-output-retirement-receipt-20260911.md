...
**The targeted output occupancy path now passes at +1.432 ns. Overall timing
improves but still fails; there is no receiver promotion or deployment.**
...
- Six new actual boundaries: shared final-word readiness withheld 128 edges, both raw resets
  during the published private hold, late unexpected FFT status, and a rejected
  final metadata word (certificate generated by the actual stage), plus
  publication vetoed despite private replay acceptance.
...
The final protocol suite passes 17 tests; the corresponding full regression
passes all 1,869 tests in 120.281 seconds. The six-case actual smoke passes in
73.658 seconds, with 150,488 output observer checks / 27 publications / 25
retirements / 27 extra holds; both pending resets cancel a private held final.
The complete actual campaign passes all 71 inherited/new boundary cases in
286.707 seconds. It checks 750,164 output cycles, 97 publications / 93 retirements
/ 97 extra holds. Fault/reset cancellation is not counted as normal retirement.
Both actual numerical CSVs retain the same SHA and 4,178-clock service.
The reference forward bank checks 1,500,484 observations with six faulting
reservations and 910 quarantined descriptor differences, as intended.
The same-source routing results are recorded below.
...
clock relaxation or receiver source changes. All 8,561 nets route without errors.

| Metric | Private descriptor parent | Output retirement |
|---|---:|---:|
...
Output occupancy D improves from -1.304 ns to **+1.432 ns**, with seven LUT
levels / 4.227 ns data delay. Prior product occupancy/identity remain passing
at +0.866 / +1.035 ns. All 70 descriptor CE pins pass, worst +1.440 ns; all 70
descriptor D pins pass, worst +3.155 ns. These are measurements of real endpoints.

Worst internal path is now product-bank metadata bit 29 through handoff/current
cutover checks to the product-bank request toggle: **-1.095 ns**, eight LUT
levels / 6.756 ns data delay, 75.711% routing. Output publication also still
fails. The first endpoint probe measured only its original toggle at -0.220 ns;
the second explicitly includes its physical replica and measures both pins,
with worst **-0.831 ns** at the replica. Both probe versions are retained; do not
use the first value as the worst output-publication path.

Overall worst is held output descriptor bit 60 crossing to slow metadata:
-1.152 ns, zero logic levels / 1.039 ns data delay under the unchanged OOC clocks.
Hold +0.070 ns and pulse +1.830 ns pass; there are zero combinational/latch loops.
114 inputs and 124 outputs remain unqualified. CDC reports nine CDC-3 / 209
CDC-15, no CDC-10. The scalar fault register directly feeds its synchronizer.
Reset-release and output-request physical replicas feed their synchronizers;
source-bank metadata bit 35 has a physical destination replica. These and the
full board clocks/reset/I/O remain unqualified, not waived.

Route: 66.503 seconds. Endpoint probes: 12.650 / 12.219 seconds. Routed DCP:
`2466665a6e1c63f8ffb9edf9dbc65041c5128652c647c48647ad6806d3205374`.
Endpoint-v2 Tcl SHA `5f449b697d0464bb220d7faae93d67095dfa6751d934470299741909922d21d2`.

Next investigate the actual product-bank publication cone, especially held-bank
identity validation feeding current cutover/fault control. Any refactoring must
preserve current publication vetoes, actual request/ACK receipts, metadata
ownership, fault quarantine and fresh-reset recovery. Prove component behavior,
run the short protocol campaign before the complete campaign, then repeat the
same route and include physical replicas in endpoint inspection. No blanket
timing waiver or removed check is justified by this result.

Only after subsystem timing and full-receiver integration/CDC/clock/reset pass
can actual 60 MS/s calibration, continuous native fine search and independent
2.5 MS/s IIO/Ethernet inspection qualify .18, then Ethernet-only .17 deployment.
