"""Record scalar fault capture with source-bound actual and physical evidence."""
import argparse
import json
...
from route_scalar_fault_sources import PIN, verify_pair
PHYSICAL_PARENT=Path('/dev/shm/starlink-sticky-fault.4qIKHja5')
...
def assess(root):
    ...
    require(sha(PARENT/'SHA256SUMS')=='9cd61fa49b0dc1313c1c6c03679a5cf0fd757cb6ac76ad8c2b8325fcf5e40f50','parent source pin')
    sys.path.insert(0,str(ROOT))
    from tests.test_starlink_scalar_fault_sources import test_exact_scalar_register_delta, TOP
    ...
    for name,count in [('component-v1.xml',18),('regression-v1.xml',1035)]:passing_tests(root,name,count)
    ...
    for folder,script in [('structure-v1','inspect_reset_receipts.tcl'),('scalar-cdc-v1','inspect_scalar_fault_cdc.tcl'),('fault-paths-v1','inspect_sticky_fault_paths.tcl'),('parent-paths-v1','inspect_sticky_fault_paths.tcl')]:
        ...
        if folder=='scalar-cdc-v1':require(r['direct_registered_fault_source']==r['first_stage_fanout_clean']=='true','direct scalar fault crossing required')
    ...
    require(counts=={'CDC-3':9,'CDC-15':208},'restored registered CDC inventory required')
    paths={}
    for folder in ['fault-paths-v1','parent-paths-v1']:
...
def package(root,output):
    ...
    name='docs/starlink-scalar-fault-sources-20260911.md';sources['current/'+name]=ROOT/name
    for p in PARENT.iterdir():
        if p.is_file():sources['reference/distributed-sticky/'+p.name]=p
    sources['reference/distributed-sticky-cdc.rpt']=PHYSICAL_PARENT/'structure-v1/cdc.rpt'
    sources['reference/distributed-sticky-timing.json']=PHYSICAL_PARENT/'route-v1/audit.json'
