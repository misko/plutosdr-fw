...
def mirror_publication_delay(bench):
    forced="force dut.product_commit_authorized=1'b0;"
    released="release dut.product_commit_authorized;"
    require(bench.count(forced)==1 and bench.count(released)==1,'exact inherited publication delay')
    return bench.replace(forced,forced+"force dut.product_other_commit_authorized=1'b0;",1).replace(released,released+"release dut.product_other_commit_authorized;",1)
...
def prepare(path,auxiliary=False):
    ...
    if auxiliary:bench=mirror_publication_delay(bench)
