...
from route_forward_private_status import evidence
...
def record(root):
    ...
    total=tests(root/'regression-v1.xml',1497)
    tests(root/'component-v2.xml',46)
    failed=ET.parse(root/'component-v1.xml').getroot()
    require(len(list(failed.iter('testcase')))==46 and len(list(failed.iter('failure')))==21,'preserve initial missing-port bench failures')
    ...
    for folder in ['actual-v1','aux-actual-v1','synthesis-v1','route-v1']:
        base=root/folder
    ...
    for pattern in ['tools/*forward_private_status*','tests/test_starlink_forward_private_status*.py','docs/starlink-forward-private-status-*.md']:
    ...
    return write_verified_archive(root/'20260911-forward-private-status-evidence.tgz',sources)
