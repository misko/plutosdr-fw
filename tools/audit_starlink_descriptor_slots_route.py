...
def audit(root):
    ...
    assert sha(run/'descriptor_slots_routed.dcp')==execution['dcp_sha256']
    ...
    assert 'checking no_input_delay (172)' in checks and 'checking no_output_delay (111)' in checks
    ...
    top=hierarchy['descriptor_slots_timing_probe'];slots=hierarchy['slots']
    assert top['ff']==slots['ff']+282 and top['ff']==524 and top['ram36']==top['ram18']==top['dsp']==0
    ...
    report=dict(dcp_sha256=execution['dcp_sha256'],source_snapshot_verified=True,
        ...
        fully_routed_nets=routed,routing_errors=errors,unconstrained_io=dict(inputs=172,outputs=111),
