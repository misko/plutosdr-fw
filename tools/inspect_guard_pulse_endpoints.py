...
def run(route,output,retimed=False):
    ...
    tcl=Path(__file__).with_name('probe_guard_pulse_endpoints.tcl')
    ...
    command=['/opt/Xilinx/Vivado/2022.2/bin/vivado','-mode','batch','-nojournal','-log',str(output/'vivado.log'),
        '-source',str(tcl),'-tclargs',str(checkpoint),routed['routed_dcp_sha'],str(output/'reports'),str(int(retimed))]
    env=dict(os.environ)
    ...
    result=dict(command=command,before=before,started=time.time(),deployment_eligible=False,retimed=retimed)
    ...
    try:
        ...
        require('RETRY_ADMISSION_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        for label in ['guard_reasons', 'admission_snapshot_valid', 'admission_snapshot_good_d', 'admission_snapshot_good_ce', 'guard_active', 'guard_ack', 'guard_commit', 'admission_consumed', 'fast_fault']:
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('route',type=Path);parser.add_argument('output',type=Path);parser.add_argument('--retimed',action='store_true')
    args=parser.parse_args();print(json.dumps(run(args.route,args.output,args.retimed),indent=2))
