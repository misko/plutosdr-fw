"""Record the same-edge forward-final decision with actual and routed evidence."""
import argparse
import json
...
from route_forward_final_commit import PIN, verify_pair
PHYSICAL_PARENT=Path('/dev/shm/starlink-ready-absorption.QxP2Z5AO')
...
def assess(root):
    ...
    require(sha(PARENT/'SHA256SUMS')=='9f97603c784ee7be2e7c4bd005bb5b2f4c4c018bdd97f9dbb674f9e18ba6bcae','parent source pin')
    sys.path.insert(0,str(ROOT))
    from tests.test_starlink_forward_final_commit import test_exact_one_guard_delta, GUARD
    ...
    require(len(names)==22 and changed==[GUARD],'bounded one-guard runtime delta')
    ...
    for name,count in [('component-v1.xml',29),('regression-v1.xml',1002),('evidence-v1.xml',7)]:passing_tests(root,name,count)
    ...
    for folder,script in [('structure-v1','inspect_reset_receipts.tcl'),('final-paths-v1','inspect_forward_final_paths.tcl'),('parent-paths-v1','inspect_forward_final_paths.tcl')]:
        r=receipt(root/folder/'receipt.txt')
        expected=sha(PHYSICAL_PARENT/'route-v1/route/retained_output_routed.dcp') if folder=='parent-paths-v1' else dcp
    ...
    paths={}
    for folder in ['final-paths-v1','parent-paths-v1']:
        values=receipt(root/folder/'paths.txt');paths[folder]=values
        for label in ['handoff_ack','handoff_active','position_active','handoff_fault']:
            require(int(values[label+'.source_count'])>=1 and values[label+'.path_count'] in {'0','1'},'bounded all-source query')
    ...
    return {'prepared_sha':PIN,'routed_dcp_sha':dcp,'runtime_modules':22,'runtime_changed':changed,
      ...
      'final_commit_witness':final_witness,'targeted_paths':paths,'cdc_counts':counts,'timing':timing,
...
def package(root,output):
    ...
    name='docs/starlink-forward-final-commit-20260911.md';sources['current/'+name]=ROOT/name
    for p in PARENT.iterdir():
        if p.is_file():sources['reference/ready-absorption/'+p.name]=p
    sources['reference/ready-absorption-cdc.rpt']=PHYSICAL_PARENT/'structure-v1/cdc.rpt'
    sources['reference/ready-absorption-timing.json']=PHYSICAL_PARENT/'route-v1/audit.json'
