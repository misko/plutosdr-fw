...
def mirror_ready_stimulus(bench):
    forced="force dut.kernel_ready=1'b0;"
    released="release dut.kernel_ready;"
    require(bench.count(forced)==2 and bench.count(released)==3,'exact inherited ready injections')
    return bench.replace(forced,forced+"force replay_kernel_reference.input_ready=1'b0;").replace(released,released+"release replay_kernel_reference.input_ready;")
...
def prepare(path,auxiliary=False):
    ...
    if auxiliary:bench=mirror_ready_stimulus(bench)
