...
import phase_publication_experiment as experiment
from staged_fft_experiment import sha,verify,require,fresh

def run(sim,synth,tests,output):
    a=json.loads((sim/'phase_publication_outcome.json').read_text())
    s=json.loads((synth/'phase_publication_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['phase_publication'],'actual full publication predicate equivalence')
    tree=ET.parse(tests)
    require(len(list(tree.iter('testcase')))==21 and not any(list(tree.iter(x)) for x in ['failure','error','skipped']),'component checks pass')
    ...
    inputs=[sim/'phase_publication_outcome.json',synth/'phase_publication_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
