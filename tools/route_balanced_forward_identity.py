"""Route balanced forward identity only after source-matched equivalence checks."""
import argparse
import json
import os
...
def evidence(actual,synthesis,aux):
    ...
    require(len(names)==31 and (prepared/'profile.tcl').read_bytes()==(aux_prepared/'profile.tcl').read_bytes(),'exact runtime profiles')
    ...
    return {'balanced_forward_identity':balanced_evidence,'product_current_fence':fence_evidence,'registered_abort':abort_evidence,'output_identity':identity_evidence,'output_boundaries':boundary_evidence,'main':a['audit'],'auxiliary':extra,'prepared':str(prepared),'prepared_sha':a['prepared_sha'],
