...
def run(route,output,expect_ring=False):
    ...
    tcl=Path(__file__).with_name('probe_replay_capacity_ring.tcl')
    ...
    command=['/opt/Xilinx/Vivado/2022.2/bin/vivado','-mode','batch','-nojournal','-log',str(output/'vivado.log'),
        '-source',str(tcl),'-tclargs',str(checkpoint),routed['routed_dcp_sha'],str(output/'reports'),str(int(expect_ring))]
    env=dict(os.environ)
    ...
    result=dict(command=command,before=before,started=time.time(),deployment_eligible=False,ring_expected=expect_ring)
    ...
    try:
        ...
        require('REPLAY_RING_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        labels=['forward_position_ce']
        if expect_ring:
            labels+=['ring_slot0_ce','ring_slot1_ce','ring_count','ring_read_slot','ring_write_slot']
        for label in labels+['kernel_next_ce','kernel_index','kernel_history','descriptor_ce','descriptor_data','occupancy','identity','output_occupancy','output_publication','product_publication']:
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('route',type=Path);parser.add_argument('output',type=Path);parser.add_argument('--ring',action='store_true')
    args=parser.parse_args();print(json.dumps(run(args.route,args.output,args.ring),indent=2))
