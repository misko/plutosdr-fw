...
def prepare(path):
    ...
    bench=bench.replace('    run_buffered_auxiliary;','    auxiliary_active=1;run_publication_cone_boundaries;',1)
...
def run(prepared,pin,output):
    ...
    result['publication_protocol']=experiment.witness((output/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text(),True)
