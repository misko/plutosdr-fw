"""Route current product fence only after the complete actual fault campaign."""
import argparse
import json
import os
...
def evidence(actual,synthesis,aux):
    ...
    require(len(names)==29 and (prepared/'profile.tcl').read_bytes()==(aux_prepared/'profile.tcl').read_bytes(),'exact runtime profiles')
    ...
    return {'product_current_fence':fence_evidence,'registered_abort':abort_evidence,'output_identity':identity_evidence,'output_boundaries':boundary_evidence,'main':a['audit'],'auxiliary':extra,'prepared':str(prepared),'prepared_sha':a['prepared_sha'],
