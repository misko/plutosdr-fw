...
def record(root):
    checked=evidence(root/'actual-v1',root/'synth-v1',root/'aux-v5')
    total=tests(root/'regression-v5.xml',1869)
    tests(root/'regression-v4.xml',1866)
    ...
    tests(root/'protocol-v2.xml',16)
    tests(root/'protocol-v3.xml',17)
    ...
    smoke=json.loads((root/'protocol-smoke-v2/smoke_outcome.json').read_text())
    require(smoke.get('passed') is True and smoke.get('full_campaign') is False,'successful bounded smoke, not full qualification')
    verify(Path(smoke['command'][-3]),smoke['prepared_sha'])
    from output_retirement_receipt_experiment_v5 import witness
    require(smoke['output_protocol']==witness((root/'protocol-smoke-v2/project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text(),True),'fresh six-case smoke witness')
    ...
    endpoints=json.loads((root/'endpoints-v2/outcome.json').read_text())
    ...
    for folder in ['prepared-v1','aux-prepared-v1','aux-prepared-v2','aux-prepared-v3','aux-prepared-v4','aux-prepared-v5',
                   'delay-probe-prepared-v1','delay-parent-prepared-v2','delay-candidate-prepared-v2',
                   'protocol-smoke-prepared-v1','protocol-smoke-prepared-v2']:
        for p in sorted((root/folder).iterdir()):
            if p.is_file():add(folder+'/'+p.name,p)
    simulation_folders=['actual-v1','aux-v1','aux-v2','aux-v3','aux-v5','delay-probe-v1','delay-parent-v2','delay-candidate-v2',
                        'protocol-smoke-v1','protocol-smoke-v2']
    for folder in simulation_folders+['synth-v1','route-v1','endpoints-v1','endpoints-v2']:
        base=root/folder
        ...
        if folder in ['endpoints-v1','endpoints-v2']:
    ...
    for folder in ['regression-v1','regression-v2','regression-v3','regression-v4','regression-v5',
                   'component-v1','component-v2','component-v3','protocol-v1','protocol-v2','protocol-v3']:
    ...
    for pattern in ['tools/*output_retirement_receipt*','tools/output_retirement_delay_probe*.py','tools/output_retirement_protocol_smoke*.py',
