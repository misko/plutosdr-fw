"""Route the integrated buffer only after source-matched actual-FFT campaigns."""
import argparse
import json
import os
...
def evidence(actual,synthesis,aux):
    ...
    require(len(names)==24 and (prepared/'profile.tcl').read_bytes()==(aux_prepared/'profile.tcl').read_bytes(),'exact runtime profiles')
    ...
    return {'main':a['audit'],'auxiliary':extra,'prepared':str(prepared),'prepared_sha':a['prepared_sha'],
