...
import replay_capacity_buffer_experiment as experiment
from staged_fft_experiment import sha,verify,require,fresh

def run(sim,synth,tests,output):
    a=json.loads((sim/'replay_capacity_outcome.json').read_text())
    s=json.loads((synth/'replay_capacity_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['replay_capacity'],'actual word conservation')
    tree=ET.parse(tests)
    ...
    inputs=[sim/'replay_capacity_outcome.json',synth/'replay_capacity_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
