# Read-only old-path and synchronizer checks; no timing exceptions.
...
foreach {label source_pattern destination_pattern} {
  reset_request {^fast_reset_fast_reg\[1\]/C$} {^output_bank/request_toggle_reg/D$}
...
foreach name {slow_reset_fast fast_reset_fast slow_reset_slow fast_reset_slow epoch_barrier/slow_purge_fast epoch_barrier/fast_release_slow} {
  set stage0 [get_cells -hier -regexp [format {^%s_reg\[0\]$} $name]]
  set stage1 [get_cells -hier -regexp [format {^%s_reg\[1\]$} $name]]
  if {[llength $stage0]!=1 || [llength $stage1]!=1} {error "reset synchronizer missing: $name"}
  foreach cell [list $stage0 $stage1] {
    if {[get_property ASYNC_REG $cell] ne "1"} {error "missing ASYNC_REG: $name"}
  }
  set endpoints [all_fanout -flat -endpoints_only -from [get_pins -of_objects $stage0 -filter {REF_PIN_NAME == Q}]]
  set required [get_pins -of_objects $stage1 -filter {REF_PIN_NAME == D}]
  if {[llength $endpoints]!=1 || [lindex $endpoints 0] ne $required} {error "unsafe reset first-stage fanout: $name"}
  puts $f "$name.first_stage_endpoints=$endpoints"
}
close $f
close_design
...
puts $f "reset_first_stage_fanout_clean=true\nconstraints_changed=false\nphysical_signoff=false\ndeployment_eligible=false"
close $f
puts MONOTONIC_RESET_PATHS_RECORDED_NOT_DEPLOYMENT_PROOF
