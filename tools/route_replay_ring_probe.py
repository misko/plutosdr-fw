...
def run(sim,synth,tests,output):
    a=json.loads((sim/'replay_ring_outcome.json').read_text())
    s=json.loads((synth/'replay_ring_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['replay_ring'],'actual word conservation')
    tree=ET.parse(tests)
    ...
    inputs=[sim/'replay_ring_outcome.json',synth/'replay_ring_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
