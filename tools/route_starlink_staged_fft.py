...
import subprocess
import time
from staged_fft_experiment import audit_publicationscope_sim, audit_preflightpublication_sim
...
def run(actual,synthesis,output):
    ...
    audited=audit_publicationscope_sim(actual) if 'publicationscope' in a['audit'] else (audit_preflightpublication_sim(actual) if 'preflightpublication' in a['audit'] else (audit_guardfacts_sim(actual) if 'guardfacts' in a['audit'] else (
