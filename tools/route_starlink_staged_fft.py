...
import subprocess
import time
from staged_fft_experiment import audit_bankidentity_sim
...
def run(actual,synthesis,output):
    ...
    audited=audit_bankidentity_sim(actual) if 'bankidentity' in a['audit'] else (audit_guardfacts_sim(actual) if 'guardfacts' in a['audit'] else (
