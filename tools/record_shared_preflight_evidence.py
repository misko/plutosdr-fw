"""Archive measured shared-preflight work without claiming receiver signoff."""
import argparse
import json
...
import shared_preflight_history_experiment as healthy
import shared_preflight_history_experiment_v2 as extended
...
def record(root,auxiliary,regression_name):
    ...
    for name in ['actual-v1','synth-v1',auxiliary]:
        receipt=json.loads((root/name/'shared_preflight_outcome.json').read_text())
    ...
    for name,checker,is_aux in [('actual-v1',healthy,False),(auxiliary,extended,True)]:
        ...
        require(checker.witness(log,is_aux)==receipts[name]['shared_preflight_history'],'fresh original-guard and direct-cause witnesses')
    reg=json.loads((root/regression_name/'outcome.json').read_text())
    require(reg.get('passed') is True and reg['tests']==2266,'complete inherited scoped regression plus new tests')
    ...
    require(len(list(tree.iter('testcase')))==2266 and not any(list(tree.iter(t)) for t in ['failure','error','skipped']),'passing test XML')
    for name in ['route-v1','endpoints-v1','preflight-endpoints-v1']:
    ...
    assessment=dict(actual=receipts,route=audit,scoped_tests=2266,whole_repository_suite=False,
                    actual_fault_cases=92,radio_access=[],primary_hdl_promoted=False,deployment_eligible=False)
    with (root/'shared-preflight-assessment.json').open('x') as out:out.write(json.dumps(assessment,indent=2)+'\n')
    sources={}
    def add(name,path):
    ...
    for folder in sorted(root.iterdir()):
        ...
        elif folder.name.startswith(('actual-','aux-v','synth-','route-','endpoints-','preflight-endpoints-')):
            for p in sorted(folder.iterdir()):
        ...
        elif folder.name.startswith(('component-','regression-')):
    ...
    add('shared-preflight-assessment.json',root/'shared-preflight-assessment.json')
    for pattern in ['tools/*shared_preflight*','tests/test_starlink_shared_preflight*',
                    'tools/*evidence_archive_stream.py']:
        for p in ROOT.glob(pattern):
            if p.is_file():add('source/'+str(p.relative_to(ROOT)),p)
    return write_verified_archive(root/'20260912-shared-preflight-evidence.tgz',sources)
