...
import retry_admission_experiment as experiment
from staged_fft_experiment import sha,verify,require,fresh

def run(sim,synth,tests,output):
    a=json.loads((sim/'retry_admission_outcome.json').read_text())
    s=json.loads((synth/'retry_admission_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['retry_admission'],'actual word conservation')
    tree=ET.parse(tests)
    ...
    inputs=[sim/'retry_admission_outcome.json',synth/'retry_admission_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
