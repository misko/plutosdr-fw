"""Route product-local framing only after source-matched actual request proof."""
import argparse
import json
import os
...
def evidence(actual,synthesis,aux):
    ...
    require(len(names)==43 and (prepared/'profile.tcl').read_bytes()==(aux_prepared/'profile.tcl').read_bytes(),'exact runtime profiles')
    ...
    for folder,is_auxiliary in [(actual,False),(aux,True)]:
        log=(folder/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text()
        receipt=json.loads((folder/'local_framing_outcome.json').read_text())
        local_evidence[folder.name]=local_framing.witness(log,is_auxiliary)
        require(receipt.get('passed') is True and receipt.get('product_local_framing')==local_evidence[folder.name],'actual request equivalence and local framing rejection')
    return {'product_local_framing':local_evidence,'private_replay_sequence':publication_evidence,'output_retirement_receipt':output_retirement_evidence,'private_forward_descriptor':descriptor_evidence,'product_retirement_receipt':retirement_evidence,'parallel_product_identity':parallel_evidence,'forward_private_status':private_evidence,'balanced_forward_identity':balanced_evidence,'product_current_fence':fence_evidence,'registered_abort':abort_evidence,'output_identity':identity_evidence,'output_boundaries':boundary_evidence,'main':a['audit'],'auxiliary':extra,'prepared':str(prepared),'prepared_sha':a['prepared_sha'],
...
def run(actual,synthesis,aux,output):
    ...
    inputs=[actual/'local_framing_outcome.json',aux/'local_framing_outcome.json',actual/'private_replay_outcome.json',aux/'private_replay_outcome.json',actual/'output_retirement_outcome.json',aux/'output_retirement_outcome.json',dcp,runner,Path(__file__).resolve(),actual/'outcome.json',synthesis/'outcome.json',aux/'outcome.json',aux/'auxiliary_outcome.json',actual/'abort_outcome.json',aux/'abort_outcome.json',actual/'identity_outcome.json',aux/'identity_outcome.json',aux/'boundary_outcome.json',actual/'product_fence_outcome.json',aux/'product_fence_outcome.json',actual/'balanced_identity_outcome.json',aux/'balanced_identity_outcome.json',actual/'private_status_outcome.json',aux/'private_status_outcome.json',actual/'parallel_identity_outcome.json',aux/'parallel_identity_outcome.json',actual/'retirement_outcome.json',aux/'retirement_outcome.json',actual/'private_descriptor_outcome.json',aux/'private_descriptor_outcome.json']
