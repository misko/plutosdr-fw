"""Route registered private abort only with source-matched fault evidence."""
import argparse
import json
import os
...
def evidence(actual,synthesis,aux):
    ...
    require(len(names)==27 and (prepared/'profile.tcl').read_bytes()==(aux_prepared/'profile.tcl').read_bytes(),'exact runtime profiles')
    ...
    return {'registered_abort':abort_evidence,'output_identity':identity_evidence,'output_boundaries':boundary_evidence,'main':a['audit'],'auxiliary':extra,'prepared':str(prepared),'prepared_sha':a['prepared_sha'],
...
def run(actual,synthesis,aux,output):
    ...
    inputs=[dcp,runner,Path(__file__).resolve(),actual/'outcome.json',synthesis/'outcome.json',aux/'outcome.json',aux/'auxiliary_outcome.json',actual/'abort_outcome.json',aux/'abort_outcome.json',actual/'identity_outcome.json',aux/'identity_outcome.json',aux/'boundary_outcome.json']
