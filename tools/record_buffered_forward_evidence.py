...
from route_buffered_forward import evidence
...
def record(root):
    ...
    total=tests(root/'regression-v1.xml',1130)+tests(root/'evidence-tests-v1.xml',29)
    ...
    for folder in ['regression-v1','evidence-tests-v1']:
    ...
    for pattern in ['tools/*buffered*.py','tests/test_starlink_buffered*.py','docs/starlink-buffered-forward-*.md']:
    ...
    return write_verified_archive(root/'20260911-buffered-forward-integration-evidence.tgz',sources)
