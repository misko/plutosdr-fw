...
def prepare(path,auxiliary=False):
    ...
    if auxiliary:sources.append(RTL/'output_retirement_receipt_boundaries_v3.svh')
    ...
    if auxiliary:
        ...
        bench=bench.replace(marker,'      run_output_retirement_boundaries;\n'+marker,1)
        bench=bench.replace('\nendmodule','\n'+(RTL/'output_retirement_receipt_boundaries_v3.svh').read_text()+'\nendmodule',1)
...
def witness(log,auxiliary=False):
    ...
    if auxiliary:
        ...
        require(cases==list(map(str,range(6))),'six actual retirement boundaries')
        require(log.count('OUTPUT_RETIREMENT_BOUNDARIES_PASS cases=6 delayed_publication=1 pending_reset=1 checked_receipt=1')==1,'retirement boundary terminal')
