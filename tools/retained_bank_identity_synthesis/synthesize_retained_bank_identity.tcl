# Exact bank-local identity runtime; unbound until new reviewed original actual success. Synthesis only; no route or timing waiver.
...
if {[file tail [info script]] ne "synthesize_retained_bank_identity.tcl"} { error "exact frozen runner filename required" }
...
set cli [file join $qualified source_snapshot tools prepare_starlink_retained_bank_identity_actual.py]
...
set status [catch {
  ...
  foreach name {synthesize_retained_bank_identity.tcl clocks.xdc threads.tcl inspect_bank_identity.tcl SHA256SUMS} {
  ...
  foreach name $compiled_names {
    if {([string match */retained_output/*.v $name] || [string match */retained_output_summary_candidate/*.v $name] || [string match */retained_output_bank_identity_candidate/*.v $name]) && ![string match */tb/* $name]} { lappend rtl_names $name }
  }
  ...
  set generics [list KERNEL_ROM_FILE=$kernel ENABLE_RETAINED_OUTPUT=1 REGISTERED_SCHEDULING=1 BOUNDARY_ROUND_SAT=1 REGISTER_OPERANDS=1 LOCAL_FIRST_ADMISSION=1 PRIVATE_DESCRIPTOR_OFFER=1 CLOSED_INPUT_CUTOVER=1 INPUT_OFFER_FAULT_SUMMARY=1 BANK_LOCAL_IDENTITY_EQ=1]
  set_property generic $generics [get_filesets sources_1]
  if {[get_property GENERIC [get_filesets sources_1]] ne $generics} { error "exact retained/R1/B1/O1/L1/private-offer/closed-input/offered-summary/bank-local-identity generics" }
...
puts RETAINED_BANK_IDENTITY_SYNTHESIS_RECORDED_NOT_TIMING_OR_DEPLOYMENT_PASS
