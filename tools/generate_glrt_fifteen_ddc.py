#!/usr/bin/env python3
"""Create the additive 15-MS/s DDC bank without replacing an existing freeze."""
...
MANIFEST = "ddc_15000000_coefficients.json"
BANK = "ddc_15000000_q17.mem"


def generate(output: Path) -> None:
    if (output / MANIFEST).exists() or (output / BANK).exists():
        raise SystemExit("15-MS/s coefficient freeze exists; do not overwrite")
    rate, taps = 15_000_000, 37
    ...
    if ripple >= 0.002 or rejection <= 78:
        raise ValueError("15-MS/s bank fails the existing DDC response limits")
