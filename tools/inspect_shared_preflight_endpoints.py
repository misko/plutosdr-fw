...
def run(route,output):
    ...
    tcl=Path(__file__).with_name('probe_shared_preflight_endpoints.tcl')
    ...
    try:
        ...
        require('SHARED_PREFLIGHT_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        for label in ['shared_history', 'private_reasons', 'guard_active', 'admission_consumed', 'fast_fault']:
