"""Short local-framing actual-FFT campaign, not inherited qualification."""
import argparse
import json
from pathlib import Path
import shutil
import product_local_framing_experiment_v2 as experiment
...
def prepare(path):
    ...
    bench=bench.replace('    run_buffered_auxiliary;','    auxiliary_active=1;product_retirement_boundary(0);run_product_local_framing_boundaries;',1)
...
def run(prepared,pin,output):
    ...
    result['local_framing_protocol']=experiment.witness((output/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text(),True)
