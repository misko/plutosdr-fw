...
def run(sim,synth,tests,output):
    a=json.loads((sim/'product_capacity_outcome.json').read_text())
    s=json.loads((synth/'product_capacity_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['product_capacity'],'actual word conservation')
    tree=ET.parse(tests)
    ...
    inputs=[sim/'product_capacity_outcome.json',synth/'product_capacity_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
