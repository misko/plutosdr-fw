...
def run(sim,synth,tests,output):
    a=json.loads((sim/'abort_echo_outcome.json').read_text())
    s=json.loads((synth/'abort_echo_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['abort_echo'],'actual mailbox and private-state equivalence')
    tree=ET.parse(tests)
    require(len(list(tree.iter('testcase')))>=30 and not any(list(tree.iter(x)) for x in ['failure','error','skipped']),'component checks pass')
    ...
    inputs=[sim/'abort_echo_outcome.json',synth/'abort_echo_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
