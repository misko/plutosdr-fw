# Exact contextual-destination runtime; independently accepted actual source. Synthesis only; no route or timing waiver.
...
if {[file tail [info script]] ne "synthesize_retained_destination.tcl"} { error "exact frozen runner filename required" }
...
set cli [file join $qualified source_snapshot tools prepare_starlink_retained_destination_actual.py]
set qualified_sha 183a0ccce954efc48d30f598d4965459695e756e20e8ca32ba47c7ee21bf3363
...
set status [catch {
  ...
  foreach name {synthesize_retained_destination.tcl clocks.xdc threads.tcl SHA256SUMS} {
  ...
  foreach name $compiled_names {
    if {([string match */retained_output/*.v $name] || [string match */retained_output_summary_candidate/*.v $name] || [string match */retained_destination_candidate/*.v $name]) && ![string match */tb/* $name] && ![string match */retained_output/starlink_pss_retained_output_owner.v $name]} { lappend rtl_names $name }
  }
  ...
  set_property top starlink_pss_fft_bank_owned_retained_destination_probe [get_filesets sources_1]
  set generics [list KERNEL_ROM_FILE=$kernel ENABLE_RETAINED_OUTPUT=1 REGISTERED_SCHEDULING=1 BOUNDARY_ROUND_SAT=1 REGISTER_OPERANDS=1 LOCAL_FIRST_ADMISSION=1 PRIVATE_DESCRIPTOR_OFFER=1 CLOSED_INPUT_CUTOVER=1 INPUT_OFFER_FAULT_SUMMARY=1 CONTEXTUAL_DESTINATION_SUMMARY=1]
  set_property generic $generics [get_filesets sources_1]
  if {[get_property GENERIC [get_filesets sources_1]] ne $generics} { error "exact retained/R1/B1/O1/L1/private-offer/closed-input/offered-summary/contextual-destination generics" }
...
puts RETAINED_DESTINATION_SYNTHESIS_RECORDED_NOT_TIMING_OR_DEPLOYMENT_PASS
