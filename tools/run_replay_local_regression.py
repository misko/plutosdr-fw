...
def run(baseline,output):
    ...
    cases=list(tree.iter('testcase'));require(len(cases)==2151,'exact inherited regression scope')
    names=sorted({x.attrib['classname'] for x in cases})
    require(len(names)==120 and all(n.startswith('tests.test_') for n in names),'exact prior modules')
    paths=[root/(n.replace('.','/')+'.py') for n in names]
    paths += [root/'tests'/('test_starlink_replay_local_fault'+suffix+'.py') for suffix in ['', '_integration','_v2']]
    ...
    receipt=dict(command=command,baseline=str(baseline),baseline_sha256=before_sha,
        test_sources=hashes,started=time.time(),scope='120 inherited modules plus 3 additive modules; not entire repository')
    ...
    receipt['passed']=receipt['returncode']==0 and receipt['tests']==2181 and receipt['sources_unchanged'] and not any(list(result.iter(x)) for x in ['failure','error','skipped'])
