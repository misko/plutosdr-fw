...
import re
import shutil
import parallel_product_identity_experiment as base
import product_retirement_receipt_transform as transform
from staged_fft_experiment import sha,require,verify
RTL=base.RTL
NEW='starlink_pss_fft_product_retirement_receipt_impl'
PARENT=Path('/dev/shm/starlink-parallel-product.70XEQYky/prepared-v1')
PIN='d6aecbc1f27dc064cfcc5a97f80748938a2aa73579afdafd8c1d07312a725d80'
...
def prepare(path,auxiliary=False):
    ...
    require(len(names)==35 and all((path/n).read_bytes()==(PARENT/n).read_bytes() for n in names),'35 parent runtime modules unchanged')
    (path/'profile.tcl').write_text(profile.replace('set runtime_names {','set runtime_names {'+NEW+'.v ',1))
    sources=[RTL/(NEW+'.v'),RTL/'product_retirement_receipt_observer.svh',Path(__file__).resolve(),Path(transform.__file__)]
    if auxiliary:sources.append(RTL/'product_retirement_receipt_boundaries.svh')
    ...
    bench=bench.replace('$fclose(log_file);','report_product_retirement_receipt;\n    $fclose(log_file);',1)
    bench=bench.replace('\nendmodule','\n'+(RTL/'product_retirement_receipt_observer.svh').read_text()+'\nendmodule',1)
    if auxiliary:
        marker='      $display("FORWARD_PRIVATE_STATUS_BOUNDARIES_PASS cases=11 private_delta_exercised=1 fresh_recovery=1");'
        require(bench.count(marker)==1,'exact auxiliary insertion')
        bench=bench.replace(marker,'      run_product_retirement_boundaries;\n'+marker,1)
        bench=bench.replace('\nendmodule','\n'+(RTL/'product_retirement_receipt_boundaries.svh').read_text()+'\nendmodule',1)
    ...
    manifest['experiment'].update(runtime_modules=36,parent_runtime_unchanged=35,product_retirement_receipt=True)
...
def witness(log,auxiliary=False):
    ...
    rows=re.findall(r'^PRODUCT_RETIREMENT_RECEIPT_PASS checks=(\d+) publications=(\d+) retires=(\d+) extra_holds=(\d+) exact_publication=1 no_post_publication_write=1 receipt_retirement=1$',log,re.M)
    require(len(rows)==1 and log.count('PRODUCT_RETIREMENT_RECEIPT_PASS')==1,'one retirement witness')
    checks,pubs,retires,holds=map(int,rows[0]);require(checks>=10000 and min(pubs,retires,holds)>=18,'nonvacuous actual retirement')
    if auxiliary:
        cases=re.findall(r'^PRODUCT_RETIREMENT_BOUNDARY_PASS boundary=(\d+) fresh_reads=512 fresh_releases=1$',log,re.M)
        require(cases==list(map(str,range(5))),'five actual retirement boundaries')
        require(log.count('PRODUCT_RETIREMENT_BOUNDARIES_PASS cases=5 delayed_publication=1 pending_reset=1 checked_receipt=1')==1,'retirement boundary terminal')
...
def run(mode,prepared,pin,output):
    ...
    try:
        if mode!='synth':result['product_retirement_receipt']=witness((output/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text(),mode=='aux')
    except Exception as error:
        result.update(passed=False,retirement_error=repr(error));raise
    finally:(output/'retirement_outcome.json').write_text(json.dumps(result,indent=2)+'\n')
    return result
