"""Archive measured retry-admission work without claiming receiver signoff."""
import argparse
import json
...
import retry_admission_experiment as healthy
import retry_admission_experiment_v7 as extended
...
def record(root,auxiliary,regression_name):
    ...
    for name in ['actual-v1','synth-v1',auxiliary]:
        receipt=json.loads((root/name/'retry_admission_outcome.json').read_text())
    ...
    require(len(names)==43 and all((main/n).read_bytes()==(aux/n).read_bytes() for n in names),'same 43 runtime files for full fault campaign')
    for name,checker,is_aux in [('actual-v1',healthy,False),(auxiliary,extended,True)]:
        ...
        require(checker.witness(log,is_aux)==receipts[name]['retry_admission'],'fresh original-guard and direct-cause witnesses')
    reg=json.loads((root/regression_name/'outcome.json').read_text())
    require(reg.get('passed') is True and reg['tests']==2299,'complete inherited scoped regression plus new tests')
    ...
    require(len(list(tree.iter('testcase')))==2299 and not any(list(tree.iter(t)) for t in ['failure','error','skipped']),'passing test XML')
    for name in ['route-v1','endpoints-v1','admission-endpoints-v2','lean-admission-endpoints-v2']:
    ...
    assessment=dict(actual=receipts,route=audit,scoped_tests=2299,whole_repository_suite=False,
                    actual_fault_cases=92,radio_access=[],primary_hdl_promoted=False,deployment_eligible=False)
    with (root/'retry-admission-assessment.json').open('x') as out:out.write(json.dumps(assessment,indent=2)+'\n')
    sources={}
    def add(name,path):
    ...
    for folder in sorted(root.iterdir()):
        ...
        elif folder.name.startswith(('actual-','aux-v','synth-','route-','endpoints-','admission-endpoints-','lean-admission-endpoints-')):
            for p in sorted(folder.iterdir()):
    ...
    add('retry-admission-assessment.json',root/'retry-admission-assessment.json')
    for pattern in ['tools/*retry_admission*','tests/test_starlink_retry_admission*',
                    'tools/*evidence_archive_stream.py']:
        for p in ROOT.glob(pattern):
            if p.is_file():add('source/'+str(p.relative_to(ROOT)),p)
    return write_verified_archive(root/'20260912-retry-admission-evidence.tgz',sources)

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('root',type=Path)
    parser.add_argument('--auxiliary',default='aux-v6');parser.add_argument('--regression',default='regression-v3')
