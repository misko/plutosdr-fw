...
def run(route,output):
    ...
    tcl=Path(__file__).with_name('probe_product_publication_cone_sources.tcl')
    ...
    try:
        ...
        require('PUBLICATION_SOURCES_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        for label in ['handoff_publication','capture_kernel','capture_publication']:
