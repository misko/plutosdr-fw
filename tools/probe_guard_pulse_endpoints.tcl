...
if {$argc != 4 || [version -short] ne "2022.2"} {error "expected ROUTED_DCP SHA NEW_OUTPUT RETIMED"}
lassign $argv checkpoint expected output retimed
if {$retimed ni {0 1}} {error "known retimed mode required"}
...
foreach {label pattern minimum maximum} {
 ...
 guard_ack *result_guard/awaiting_ack_reg/D 2 2
 guard_commit *result_guard/commit_pulse_reg/D 2 2
 admission_consumed *admission_gate/consumed_reg/D 1 1
 fast_fault *fast_fault_reg/D 1 1
} {
 if {$retimed && $label eq "guard_active"} {set pattern *result_guard/active_storage_reg/D}
 if {$retimed && $label eq "guard_ack"} {set pattern *result_guard/ack_storage_reg/D}
