...
def run(route,output,expect_fifo=False):
    ...
    tcl=Path(__file__).with_name('probe_replay_capacity_buffer_v2.tcl')
    ...
    command=['/opt/Xilinx/Vivado/2022.2/bin/vivado','-mode','batch','-nojournal','-log',str(output/'vivado.log'),
        '-source',str(tcl),'-tclargs',str(checkpoint),routed['routed_dcp_sha'],str(output/'reports'),str(int(expect_fifo))]
    env=dict(os.environ)
    ...
    result=dict(command=command,before=before,started=time.time(),deployment_eligible=False,fifo_expected=expect_fifo)
    ...
    try:
        ...
        require('REPLAY_CAPACITY_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        labels=['forward_position_ce']
        if expect_fifo:
            labels+=['fifo_head_ce','fifo_tail_ce','fifo_head_valid','fifo_tail_valid']
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('route',type=Path);parser.add_argument('output',type=Path);parser.add_argument('--fifo',action='store_true')
    args=parser.parse_args();print(json.dumps(run(args.route,args.output,args.fifo),indent=2))
