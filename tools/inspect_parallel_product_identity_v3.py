...
def run(route,output):
    ...
    tcl=Path(__file__).with_name('probe_parallel_product_identity_v3.tcl')
    ...
    try:
        ...
        path=(output/'reports/selector_paths.rpt').read_text()
        require('product_identity_stage/output_identity_good_reg' in path,'observed actual certificate endpoint')
        result.update(passed=True,comparators=2,selector_slack_ns=float(re.search(r'^selector.slack=([-\d.]+)$',report,re.M)[1]))
