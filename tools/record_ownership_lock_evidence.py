"""Archive measured ownership-lock work without claiming receiver signoff."""
import argparse
import json
...
import ownership_lock_experiment as healthy
import ownership_lock_experiment as extended
...
def record(root,auxiliary,regression_name):
    ...
    for name in ['actual-v1','synth-v1',auxiliary]:
        receipt=json.loads((root/name/'ownership_lock_outcome.json').read_text())
    ...
    require(len(names)==44 and all((main/n).read_bytes()==(aux/n).read_bytes() for n in names),'same 44 runtime files for full fault campaign')
    for name,checker,is_aux in [('actual-v1',healthy,False),(auxiliary,extended,True)]:
        ...
        require(checker.witness(log,is_aux)==receipts[name]['ownership_lock'],'fresh original-lock witnesses')
    reg=json.loads((root/regression_name/'outcome.json').read_text())
    require(reg.get('passed') is True and reg['tests']==2330,'complete inherited scoped regression plus new tests')
    ...
    require(len(list(tree.iter('testcase')))==2330 and not any(list(tree.iter(t)) for t in ['failure','error','skipped']),'passing test XML')
    for name in ['route-v1','endpoints-v1','admission-endpoints-v2','parent-admission-endpoints-v2']:
    ...
    assessment=dict(actual=receipts,route=audit,scoped_tests=2330,whole_repository_suite=False,
                    actual_fault_cases=92,radio_access=[],primary_hdl_promoted=False,deployment_eligible=False)
    with (root/'ownership-lock-assessment.json').open('x') as out:out.write(json.dumps(assessment,indent=2)+'\n')
    sources={}
    def add(name,path):
    ...
    add('pin-enumeration.log',root/'pin-enumeration.log')
    add('ownership-lock-assessment.json',root/'ownership-lock-assessment.json')
    for pattern in ['tools/*ownership_lock*','tests/test_starlink_ownership_lock*',
                    'tools/*evidence_archive_stream.py']:
        for p in ROOT.glob(pattern):
            if p.is_file():add('source/'+str(p.relative_to(ROOT)),p)
    return write_verified_archive(root/'20260912-ownership-lock-evidence.tgz',sources)

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('root',type=Path)
    parser.add_argument('--auxiliary',default='aux-v1');parser.add_argument('--regression',default='regression-v2')
