#!/usr/bin/env python3
"""Generate bit-exact 30 MS/s DDC-to-XFFT end-to-end replay vectors."""
...
SCHEMA = "starlink-pss30-ddc-to-xfft-v1"
DATA_BITS = 18
SOURCE_SAMPLE_COUNT = 2826
ACQUISITION_SAMPLE_COUNT = 1406
BLOCK_COUNT = 3
SCORE_COUNT = BLOCK_COUNT * XFFT_VALID_OUTPUTS
FIRST_SOURCE_INDEX = 2_000_000
FIRST_ACQUISITION_INDEX = 1_000_004
...
def generate(output_directory: Path) -> dict[str, object]:
    ...
    source_pss = quantize_q15(projected_pss(30_000_000, "upper"))
    source_pss_starts: list[int] = []
    for score_offset in PSS_SCORE_OFFSETS:
        source_start = 8 + 2 * score_offset
        source_pss_starts.append(source_start)
        source_samples[source_start : source_start + 132] = source_pss
    ...
    with TemporaryDirectory(prefix="starlink-pss30-ddc-xfft-") as temporary:
    ...
    evidence: dict[str, object] = {
        ...
        "source_sample_rate_hz": 30_000_000,
    ...
    print(
        "STARLINK_PSS30_DDC_XFFT_VECTORS_PASS "
        f"source={SOURCE_SAMPLE_COUNT} acquisition={ACQUISITION_SAMPLE_COUNT} "
        f"scores={SCORE_COUNT} pss255=3 kernel={result.kernel_sha256}"
    )
    return evidence
