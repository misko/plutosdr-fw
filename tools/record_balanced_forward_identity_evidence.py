...
from route_balanced_forward_identity import evidence
...
def record(root):
    ...
    total=tests(root/'regression-v1.xml',1381)+tests(root/'route-tests-v1.xml',31)
    ...
    netlist=json.loads((root/'netlist-v1/outcome.json').read_text())
    require(netlist.get('passed') is True and netlist.get('sources_unchanged') is True and netlist.get('carry_cells')==0,'implemented balanced comparison')
    ...
    for folder in ['actual-v1','aux-actual-v1','synth-v1','route-v1','netlist-v1']:
        base=root/folder
    ...
    for folder in ['regression-v1','route-tests-v1','component-v1']:
    ...
    for pattern in ['tools/*balanced_forward_identity*','tests/test_starlink_balanced_forward_identity*.py','docs/starlink-balanced-forward-identity-*.md']:
    ...
    return write_verified_archive(root/'20260911-balanced-forward-identity-evidence.tgz',sources)
