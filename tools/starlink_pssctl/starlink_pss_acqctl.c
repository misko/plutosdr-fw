...
#define DEFAULT_MONITOR_DURATION_MS 120000U
#define MAX_MONITOR_DURATION_MS 120000U
...
static void usage(FILE *stream)
{
	fprintf(stream,
		...
		"  monitor [--duration-ms N] [--timeout-ms N]\n"
		"\n"
		...
		"or frame-lock claim. monitor continuously copies and releases every\n"
		"15 MS/s map for at most 120 seconds, emits one NDJSON record per map\n"
		"and one final summary, and makes no PSS-detection or frame-lock claim.\n",
...
static int parse_monitor_duration(const char *text, unsigned int *value)
{
	char *end;
	unsigned long parsed;
	...
	errno = 0;
	parsed = strtoul(text, &end, 0);
	if (errno || *end || parsed < 1000UL ||
	    parsed > MAX_MONITOR_DURATION_MS)
		return -1;
	*value = (unsigned int)parsed;
	return 0;
}
...
static int print_monitor_map(const char *serial, uint32_t sequence,
...
static void print_monitor_summary(const char *serial,
	...
	const struct ddc_counters *ddc_after)
{
	...
	printf("{\"schema\":\"starlink-pss-acqctl.monitor-summary.v1\","
		...
		initial_snapshot->accepted_score_count,
	       ...
	       cutoff->discarded_score_count, cutoff->discontinuity_abort_count,
	...
	unsigned int timeout_ms, char *error, size_t error_size)
{
	...
	if (input_rate_msps(info) != 15U) {
		snprintf(error, error_size,
			"monitor v1 qualifies only the canonical 15 MS/s input");
		return -1;
	}
	if (info->status & PSS_MAP_STATUS_ENABLED) {
		snprintf(error, error_size,
			"monitor requires the acquisition engine to be disabled");
		return -1;
	}
	storage = calloc(PSS_ACQUISITION_WINDOW_MAPS * PSS_MAP_PHASE_BINS,
		sizeof(*storage));
	...
	if (pss_map_take_snapshot(io, &initial_snapshot, timeout_ms,
		...
		snprintf(error, error_size,
			"cannot establish one clean monitor epoch");
		goto done;
	}
	if (ddc_before.discontinuity || ddc_before.saturation) {
		snprintf(error, error_size,
			"DDC fault counters were nonzero before monitor start");
		goto done;
	}
	if (setvbuf(stdout, NULL, _IOLBF, 0) != 0) {
		snprintf(error, error_size, "cannot enable line-buffered monitor output");
		goto done;
	}
	...
	for (;;) {
		...
		if (interrupted) {
			snprintf(error, error_size, "continuous monitor interrupted");
			goto done;
		}
		if (pss_map_wait_copy(io, incoming, PSS_MAP_PHASE_BINS,
		...
		if (maps_copied && !pss_map_copies_contiguous(&previous_copy,
				&current_copy)) {
			snprintf(error, error_size,
				"continuous phase-map copies were not contiguous");
			goto done;
		}
		if (pss_map_window_push(&window, incoming,
		...
		if (print_monitor_map(serial, maps_copied, &current_copy,
				published_candidate) < 0) {
			snprintf(error, error_size, "cannot write monitor output");
			goto done;
		}
	...
	if (pss_map_take_snapshot(io, &final_snapshot, timeout_ms,
		...
		snprintf(error, error_size, "cannot capture final monitor counters");
		goto done;
	}
	if (maps_copied < PSS_ACQUISITION_WINDOW_MAPS ||
	    ...
	    ddc_after.discontinuity || ddc_after.saturation) {
		snprintf(error, error_size,
			"monitor final continuity or counter gate failed");
		goto done;
	}
	if (pss_map_set_enabled(io, false, true, error, error_size) < 0)
		goto done;
	enabled = false;
	print_monitor_summary(serial, duration_ms, now_ms - started_ms,
		maps_copied, candidate_windows, first_generation,
		first_start_index, &previous_copy, &initial_snapshot,
		&final_snapshot, &ddc_before, &ddc_after);
	if (fflush(stdout) < 0) {
		snprintf(error, error_size, "cannot flush monitor summary");
		goto done;
	}
	result = 0;
	...
	if (enabled) {
		...
		if (pss_map_set_enabled(io, false, true,
			...
			if (!io->write32 || io->write32(io->context,
				...
				fprintf(stderr,
					"monitor cleanup required direct disable/flush fallback: %s\n",
					cleanup_error);
			result = -1;
		}
	}
...
int main(int argc, char **argv)
{
	...
	unsigned int monitor_duration_ms = DEFAULT_MONITOR_DURATION_MS;
	int argument = 1;
		...
		while (argument < argc) {
			...
			if (!strcmp(argv[argument], "--timeout-ms")) {
				if (option_value(argc, argv, &argument, &value) < 0 ||
					...
					fprintf(stderr, "invalid monitor timeout option\n");
					return EXIT_FAILURE;
				}
			} else if (!strcmp(argv[argument], "--duration-ms")) {
				if (option_value(argc, argv, &argument, &value) < 0 ||
				    parse_monitor_duration(value,
					&monitor_duration_ms) < 0) {
					fprintf(stderr, "invalid monitor duration option\n");
					return EXIT_FAILURE;
				}
			} else {
				fprintf(stderr, "invalid monitor option\n");
				return EXIT_FAILURE;
			}
			++argument;
		}
	} else if (!strcmp(command, "snapshot") || !strcmp(command, "candidate") ||
	...
	if (!strcmp(command, "info")) {
		...
		if (run_monitor(expected_serial, &io, &info,
				monitor_duration_ms, timeout_ms,
				error, sizeof(error)) < 0) {
			fprintf(stderr, "continuous monitor failed: %s\n", error);
			goto done;
		}
		return_code = EXIT_SUCCESS;
	}
