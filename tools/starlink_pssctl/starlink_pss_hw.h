...
/*
 * Keep the qualified 15 MS/s executable byte-contract as the default, while
 * allowing the same fail-closed source to be compiled for one immutable
 * higher-rate ABI.  A binary still accepts exactly one rate and geometry; it
 * never negotiates or silently broadens its hardware contract at run time.
 */
#ifndef PSS_BUILD_RATE_MSPS
#define PSS_BUILD_RATE_MSPS 15
#endif

#if PSS_BUILD_RATE_MSPS == 15
...
#define PSS_RATE_MULTIPLIER 1U
#define PSS_COEFFICIENT_COUNT_WIDTH 7U
#define PSS_HAS_INJECTION 1
#elif PSS_BUILD_RATE_MSPS == 30
#define PSS_VERSION UINT32_C(0x00010003)
#define PSS_RATE_MSPS UINT32_C(30)
#define PSS_GEOMETRY UINT32_C(0x0bca0884)
#define PSS_CAPABILITIES UINT32_C(0x0000001d)
#define PSS_RATE_MULTIPLIER 2U
#define PSS_COEFFICIENT_COUNT_WIDTH 8U
#define PSS_HAS_INJECTION 0
#elif PSS_BUILD_RATE_MSPS == 60
#define PSS_VERSION UINT32_C(0x00010003)
#define PSS_RATE_MSPS UINT32_C(60)
#define PSS_GEOMETRY UINT32_C(0x0f8c1108)
#define PSS_CAPABILITIES UINT32_C(0x0000001d)
#define PSS_RATE_MULTIPLIER 4U
#define PSS_COEFFICIENT_COUNT_WIDTH 9U
#define PSS_HAS_INJECTION 0
#else
#error "PSS_BUILD_RATE_MSPS must be exactly 15, 30, or 60"
#endif

#define PSS_COEFFICIENT_COUNT (66U * PSS_RATE_MULTIPLIER)
#define PSS_CAPTURE_SAMPLES (130U * PSS_RATE_MULTIPLIER)
#define PSS_QUALIFIED_LAG_COUNT (60U * PSS_RATE_MULTIPLIER + 1U)
#define PSS_RESULT_WORDS 26U
#define PSS_INJECTION_SAMPLES 130U
#define PSS_FIRST_LAG (-30 * (int)PSS_RATE_MULTIPLIER)
#define PSS_LAST_LAG (30 * (int)PSS_RATE_MULTIPLIER)
#define PSS_FRAME_PERIOD_SAMPLES (20000U * PSS_RATE_MULTIPLIER)
#define PSS_DEFAULT_LEAD_SAMPLES \
	(UINT64_C(1000000) * PSS_RATE_MULTIPLIER)
#define PSS_MINIMUM_HOST_LEAD (UINT64_C(65536) * PSS_RATE_MULTIPLIER)
#define PSS_COMMAND_FIFO_USABLE 7U
#define PSS_DEFAULT_QUEUE_TARGET 7U
#define PSS_MAX_BATCH_COUNT UINT32_C(1000000)
#define PSS_STATUS_COEFFICIENT_COUNT_MASK \
	((UINT32_C(1) << PSS_COEFFICIENT_COUNT_WIDTH) - UINT32_C(1))
#define PSS_STATUS_QUEUE_ROOM_SHIFT (10U + PSS_COEFFICIENT_COUNT_WIDTH)
