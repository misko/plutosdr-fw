...
def run(baseline,output):
    ...
    cases=list(tree.iter('testcase'));require(len(cases)==2390,'exact inherited regression scope')
    names=sorted({x.attrib['classname'] for x in cases})
    require(len(names)==135 and all(n.startswith('tests.test_') for n in names),'exact prior modules')
    paths=[root/(n.replace('.','/')+'.py') for n in names]
    paths += [root/'tests'/name for name in ['test_starlink_phase_publication.py']]
    ...
    receipt=dict(command=command,baseline=str(baseline),baseline_sha256=before_sha,
        test_sources=hashes,started=time.time(),scope='135 inherited modules plus 1 phase-publication module; not entire repository')
    ...
    receipt['passed']=receipt['returncode']==0 and receipt['tests']==2411 and receipt['sources_unchanged'] and not any(list(result.iter(x)) for x in ['failure','error','skipped'])
