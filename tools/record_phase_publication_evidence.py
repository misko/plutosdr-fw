"""Archive phase-publication evidence without claiming receiver signoff."""
import argparse
import json
...
import phase_publication_experiment as healthy
import phase_publication_experiment as extended
...
def record(root,auxiliary,regression_name):
    ...
    for name in ['actual-v1','synth-v1',auxiliary]:
        receipt=json.loads((root/name/'phase_publication_outcome.json').read_text())
    ...
    require(len(names)==46 and all((main/n).read_bytes()==(aux/n).read_bytes() for n in names),'same 46 runtime files for full fault campaign')
    for name,checker,is_aux in [('actual-v1',healthy,False),(auxiliary,extended,True)]:
        ...
        require(checker.witness(log,is_aux)==receipts[name]['phase_publication'],'fresh original publication witnesses')
        require(checker.base.witness(log,is_aux)==receipts[name]['guard_pulse'],'fresh original guards')
        require(checker.base.base.witness(log,is_aux)==receipts[name]['retry_admission'],'fresh original admission and ten boundaries')
    reg=json.loads((root/regression_name/'outcome.json').read_text())
    require(reg.get('passed') is True and reg['tests']==2411,'complete inherited scoped regression plus new tests')
    ...
    require(len(list(tree.iter('testcase')))==2411 and not any(list(tree.iter(t)) for t in ['failure','error','skipped']),'passing test XML')
    ...
    require(len(list(component.iter('testcase')))==21 and not any(list(component.iter(t)) for t in ['failure','error','skipped']),'21 component tests pass')
    assessment=dict(actual=receipts,route=audit,scoped_tests=2411,whole_repository_suite=False,
                    actual_fault_cases=92,radio_access=[],primary_hdl_promoted=False,deployment_eligible=False)
    with (root/'phase-publication-assessment.json').open('x') as out:out.write(json.dumps(assessment,indent=2)+'\n')
    sources={}
    def add(name,path):
    ...
    add('phase-publication-assessment.json',root/'phase-publication-assessment.json')
    for pattern in ['tools/*phase_publication*','tests/test_starlink_phase_publication*',
                    'tools/*evidence_archive_stream.py']:
        for p in ROOT.glob(pattern):
            if p.is_file():add('source/'+str(p.relative_to(ROOT)),p)
    return write_verified_archive(root/'20260912-phase-publication-evidence.tgz',sources)
