...
def run(sim,synth,tests,output):
    a=json.loads((sim/'replay_local_fault_outcome.json').read_text())
    s=json.loads((synth/'replay_local_fault_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['replay_local_fault'],'actual word conservation')
    tree=ET.parse(tests)
    ...
    inputs=[sim/'replay_local_fault_outcome.json',synth/'replay_local_fault_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
