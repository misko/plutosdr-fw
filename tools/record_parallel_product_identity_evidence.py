...
from route_parallel_product_identity import evidence
...
def record(root):
    checked=evidence(root/'actual-v1',root/'synthesis-v1',root/'aux-actual-v1')
    total=tests(root/'regression-v1.xml',1565)
    tests(root/'component-v2.xml',12)
    failed=ET.parse(root/'component-v1.xml').getroot()
    require(len(list(failed.iter('testcase')))==12 and len(list(failed.iter('failure')))==6,'preserve initial test-adapter mismatch failures')
    ...
    netlist=json.loads((root/'netlist-v4/outcome.json').read_text())
    require(netlist.get('passed') is True and netlist.get('sources_unchanged') is True and netlist.get('comparators')==2,'implemented parallel comparisons')
    require(all(sha(Path(p))==value for p,value in netlist['before'].items()),'netlist source receipts')
    for version in [1,2,3]:
        prior=json.loads((root/f'netlist-v{version}/outcome.json').read_text())
        require(prior.get('passed') is False and prior.get('sources_unchanged') is True,'retain rejected observational query')
        require(all(sha(Path(p))==value for p,value in prior['before'].items()),'old observational query source pins')
    assessment={'netlist':netlist,'source_evidence':checked,'tests':total,'route':route,
    ...
    for folder in ['actual-v1','aux-actual-v1','synthesis-v1','route-v1','netlist-v1','netlist-v2','netlist-v3','netlist-v4']:
        base=root/folder
    ...
    for folder in ['regression-v1','component-v1','component-v2']:
    ...
    for pattern in ['tools/*parallel_product_identity*','tests/test_starlink_parallel_product_identity*.py','docs/starlink-parallel-product-identity-*.md']:
    ...
    return write_verified_archive(root/'20260911-parallel-product-identity-evidence.tgz',sources)
