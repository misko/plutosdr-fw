...
def run(baseline,output):
    ...
    cases=list(tree.iter('testcase'));require(len(cases)==2411,'exact inherited regression scope')
    names=sorted({x.attrib['classname'] for x in cases})
    require(len(names)==136 and all(n.startswith('tests.test_') for n in names),'exact prior modules')
    paths=[root/(n.replace('.','/')+'.py') for n in names]
    paths += [root/'tests'/name for name in ['test_starlink_product_publication.py']]
    ...
    receipt=dict(command=command,baseline=str(baseline),baseline_sha256=before_sha,
        test_sources=hashes,started=time.time(),scope='136 inherited modules plus 1 product-publication module; not entire repository')
    ...
    receipt['passed']=receipt['returncode']==0 and receipt['tests']==2431 and receipt['sources_unchanged'] and not any(list(result.iter(x)) for x in ['failure','error','skipped'])
