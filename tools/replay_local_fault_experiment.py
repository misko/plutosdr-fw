...
def prepare(path,auxiliary=False):
    ...
    for source in [RTL/(NEW+'.v'),RTL/(BUFFER+'.v'),RTL/'replay_local_fault_observer.svh',Path(__file__).resolve(),Path(transform.__file__)]:
    ...
    bench=bench.replace('\nendmodule','\n'+(RTL/'replay_local_fault_observer.svh').read_text()+'\nendmodule',1)
...
def witness(log,auxiliary=False):
    ...
    rows=re.findall(r'^REPLAY_LOCAL_FAULT_PASS checks=(\d+) external_only=(\d+) current_veto=1 registered_cause=1$',log,re.M)
    require(len(rows)==1 and log.count('REPLAY_LOCAL_FAULT_PASS')==1,'one actual fault partition witness')
