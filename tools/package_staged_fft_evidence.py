...
def package(output,campaign='output'):
    ...
    if campaign not in {'output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:raise ValueError('explicit campaign required')
    final_version={'output':5,'handover':4,'admission':3,'completion':2,'replay':2,'capture':1,'finalcapture':1,'sequence':1,'certification':1,'guardfacts':1,'heldmeta':1,'heldhandoff':1,'privateack':1,'privateinput':1}[campaign]
    sources={}
    def add(name,path):
    ...
    for version in ({'output':[5],'handover':[2,4],'admission':[1,2,3],'completion':[1,2],'replay':[1,2],'capture':[1],'finalcapture':[1],'sequence':[1],'certification':[1],'guardfacts':[1],'heldmeta':[1],'heldhandoff':[1],'privateack':[1],'privateinput':[1]}[campaign]):
        ...
        for name in ['staged-completion-lint-v1','staged-completion-unit-v1','staged-completion-regression-v1',
            ...
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
                    add('tests/'+name+'/'+str(p.relative_to(folder)),p)
            add('tests/'+name+'.xml',RECOVERY/(name+'.xml'))
    if campaign in {'replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
        names=(['staged-privateinput-unit-v1','staged-privateinput-premise-v1','staged-privateinput-regression-v1'] if campaign=='privateinput' else
               ['staged-privateack-unit-v1','staged-privateack-accounting-v1','staged-privateack-regression-v1'] if campaign=='privateack' else
    ...
    for p in sorted((ROOT/'tests').glob('test_starlink*')):
        if p.name in {'test_starlink_descriptor_commands_mailbox.py','test_starlink_staged_mailbox_control.py',
                      ...
                      'test_starlink_private_input.py','test_starlink_privateinput_audit.py'}:
    ...
    if campaign in {'handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
        add('current/docs/starlink-staged-admission-20260911.md',ROOT/'docs/starlink-staged-admission-20260911.md')
    if campaign in {'completion','replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'guardfacts','heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'heldmeta','heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'heldhandoff','privateack','privateinput'}:
    ...
    if campaign in {'privateack','privateinput'}:
    ...
    if campaign=='privateinput':
        add('current/docs/starlink-private-input-observations-20260911.md',ROOT/'docs/starlink-private-input-observations-20260911.md')
        for name in ['starlink_pss_fft_staged_output_impl.v','starlink_pss_result_guard_owner_view.v','starlink_pss_realtime_input_guard_local_admission.v']:
            add('reference/privateack/'+name,RECOVERY/'staged-privateack-prepared-v1'/name)
...
if __name__=='__main__':
    ...
    parser.add_argument('--campaign',choices=['output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','heldmeta','heldhandoff','privateack','privateinput'],default='output')
