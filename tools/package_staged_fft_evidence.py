...
def package(output,campaign='output'):
    ...
    if campaign not in {'output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:raise ValueError('explicit campaign required')
    final_version={'output':5,'handover':4,'admission':3,'completion':2,'replay':2,'capture':1,'finalcapture':1,'sequence':1,'certification':1,'guardfacts':1,'preflightpublication':1,'publicationscope':2}[campaign]
    sources={}
    def add(name,path):
    ...
    for version in ({'output':[5],'handover':[2,4],'admission':[1,2,3],'completion':[1,2],'replay':[1,2],'capture':[1],'finalcapture':[1],'sequence':[1],'certification':[1],'guardfacts':[1],'preflightpublication':[1],'publicationscope':[1,2]}[campaign]):
        for kind in ['synth','route']:
            if campaign in {'admission','publicationscope'} and version==1 and kind=='route':continue
        ...
        for name in ['staged-completion-lint-v1','staged-completion-unit-v1','staged-completion-regression-v1',
            ...
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
                    add('tests/'+name+'/'+str(p.relative_to(folder)),p)
            add('tests/'+name+'.xml',RECOVERY/(name+'.xml'))
    if campaign in {'replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
        names=(['staged-publicationscope-unit-v1','staged-publicationscope-harness-v2','staged-publicationscope-regression-v2'] if campaign=='publicationscope' else
    ...
    for p in sorted((ROOT/'tests').glob('test_starlink*')):
        if p.name in {'test_starlink_descriptor_commands_mailbox.py','test_starlink_staged_mailbox_control.py',
                      ...
                      'test_starlink_preflight_publication.py','test_starlink_preflightpublication_audit.py',
                      'test_starlink_publication_fault_scope.py','test_starlink_publicationscope_audit.py'}:
    ...
    if campaign in {'handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
        add('current/docs/starlink-staged-admission-20260911.md',ROOT/'docs/starlink-staged-admission-20260911.md')
    if campaign in {'completion','replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'sequence','certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'certification','guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign in {'guardfacts','preflightpublication','publicationscope'}:
    ...
    if campaign=='publicationscope':
        add('current/docs/starlink-publication-fault-scope-20260911.md',ROOT/'docs/starlink-publication-fault-scope-20260911.md')
        add('current/docs/starlink-preflight-publication-proof-20260911.md',ROOT/'docs/starlink-preflight-publication-proof-20260911.md')
        parent=RECOVERY/'staged-preflightpublication-prepared-v1'
        runtime_names=(parent/'profile.tcl').read_text().split('set runtime_names {')[1].split('}')[0].split()
        if len(runtime_names)!=17:raise ValueError('exact publication-order parent runtime')
        for name in runtime_names:
            add('reference/preflight-runtime/'+name,parent/name)
...
if __name__=='__main__':
    ...
    parser.add_argument('--campaign',choices=['output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','preflightpublication','publicationscope'],default='output')
