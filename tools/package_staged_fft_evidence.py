...
def package(output,campaign='output'):
    ...
    if campaign not in {'output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:raise ValueError('explicit campaign required')
    final_version={'output':5,'handover':4,'admission':3,'completion':2,'replay':2,'capture':1,'finalcapture':1,'sequence':1,'certification':1,'guardfacts':1,'bankidentity':1}[campaign]
    sources={}
    def add(name,path):
    ...
    for version in ({'output':[5],'handover':[2,4],'admission':[1,2,3],'completion':[1,2],'replay':[1,2],'capture':[1],'finalcapture':[1],'sequence':[1],'certification':[1],'guardfacts':[1],'bankidentity':[1]}[campaign]):
        ...
        for name in ['staged-completion-lint-v1','staged-completion-unit-v1','staged-completion-regression-v1',
            ...
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
                    add('tests/'+name+'/'+str(p.relative_to(folder)),p)
            add('tests/'+name+'.xml',RECOVERY/(name+'.xml'))
    if campaign in {'replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:
        names=(['staged-bankidentity-unit-v1','staged-bankidentity-regression-v1'] if campaign=='bankidentity' else
    ...
    for p in sorted((ROOT/'tests').glob('test_starlink*')):
        if p.name in {'test_starlink_descriptor_commands_mailbox.py','test_starlink_staged_mailbox_control.py',
                      ...
                      'test_starlink_bank_local_identity.py','test_starlink_bankidentity_audit.py'}:
    ...
    if campaign in {'handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:
        add('current/docs/starlink-staged-admission-20260911.md',ROOT/'docs/starlink-staged-admission-20260911.md')
    if campaign in {'completion','replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'capture','finalcapture','sequence','certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'finalcapture','sequence','certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'sequence','certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'certification','guardfacts','bankidentity'}:
    ...
    if campaign in {'guardfacts','bankidentity'}:
    ...
    if campaign=='bankidentity':
        add('current/docs/starlink-bank-local-identity-20260911.md',ROOT/'docs/starlink-bank-local-identity-20260911.md')
        for name in ['starlink_pss_fft_staged_output_impl.v','starlink_pss_result_guard_owner_view.v','starlink_pss_realtime_input_guard_local_admission.v']:
            add('reference/guardfacts/'+name,RECOVERY/'staged-guardfacts-prepared-v1'/name)
...
if __name__=='__main__':
    ...
    parser.add_argument('--campaign',choices=['output','handover','admission','completion','replay','capture','finalcapture','sequence','certification','guardfacts','bankidentity'],default='output')
