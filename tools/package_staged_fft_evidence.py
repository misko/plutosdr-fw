...
def package(output,campaign='output'):
    ...
    if campaign not in {'output','handover','admission','completion','replay','capture','final'}:raise ValueError('explicit campaign required')
    final_version={'output':5,'handover':4,'admission':3,'completion':2,'replay':2,'capture':1,'final':3}[campaign]
    sources={}
    def add(name,path):
    ...
    for version in ({'output':[5],'handover':[2,4],'admission':[1,2,3],'completion':[1,2],'replay':[1,2],'capture':[1],'final':[1,2,3]}[campaign]):
        for kind in ['synth','route']:
            if kind=='route' and ((campaign=='admission' and version==1) or (campaign=='final' and version<3)):continue
        ...
        for name in ['staged-completion-lint-v1','staged-completion-unit-v1','staged-completion-regression-v1',
            ...
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
                    add('tests/'+name+'/'+str(p.relative_to(folder)),p)
            add('tests/'+name+'.xml',RECOVERY/(name+'.xml'))
    if campaign in {'replay','capture','final'}:
        names=(['staged-replay-preflight-v1','staged-replay-unit-v1','staged-replay-regression-v1',
               ...
               ['staged-final-preflight-v1','staged-final-unit-v1','staged-final-regression-v3'])
        for name in names:
            folder=RECOVERY/name
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
    ...
    for p in sorted((ROOT/'tests').glob('test_starlink*')):
        if p.name in {'test_starlink_descriptor_commands_mailbox.py','test_starlink_staged_mailbox_control.py',
                      ...
                      'test_starlink_private_descriptor_capture.py','test_starlink_capture_audit.py','test_starlink_final_audit.py'}:
    ...
    if campaign in {'handover','admission','completion','replay','capture','final'}:
    ...
    if campaign in {'admission','completion','replay','capture','final'}:
        add('current/docs/starlink-staged-admission-20260911.md',ROOT/'docs/starlink-staged-admission-20260911.md')
    if campaign in {'completion','replay','capture','final'}:
    ...
    if campaign=='final':
        add('current/docs/starlink-staged-final-20260911.md',ROOT/'docs/starlink-staged-final-20260911.md')
...
if __name__=='__main__':
    ...
    parser.add_argument('--campaign',choices=['output','handover','admission','completion','replay','capture','final'],default='output')
