...
def package(output,campaign='output'):
    ...
    if campaign not in {'output','handover','admission','completion','replay','capture','ordinal'}:raise ValueError('explicit campaign required')
    final_version={'output':5,'handover':4,'admission':3,'completion':2,'replay':2,'capture':1,'ordinal':1}[campaign]
    sources={}
    def add(name,path):
    ...
    for version in ({'output':[5],'handover':[2,4],'admission':[1,2,3],'completion':[1,2],'replay':[1,2],'capture':[1],'ordinal':[1]}[campaign]):
        ...
        for name in ['staged-completion-lint-v1','staged-completion-unit-v1','staged-completion-regression-v1',
            ...
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
                    add('tests/'+name+'/'+str(p.relative_to(folder)),p)
            add('tests/'+name+'.xml',RECOVERY/(name+'.xml'))
    if campaign in {'replay','capture','ordinal'}:
        names=(['staged-replay-preflight-v1','staged-replay-unit-v1','staged-replay-regression-v1',
               ...
               ['staged-capture-preflight-v1','staged-capture-unit-v1','staged-capture-unit-v2','staged-capture-regression-v1'] if campaign=='capture' else
               ['staged-ordinal-preflight-v1','staged-ordinal-regression-v1'])
        for name in names:
            folder=RECOVERY/name
            for p in sorted(folder.rglob('*')):
                if p.is_file() and p.suffix in {'.xml','.log','.json','.sv','.v','.txt'}:
    ...
    for p in sorted((ROOT/'tests').glob('test_starlink*')):
        if p.name in {'test_starlink_descriptor_commands_mailbox.py','test_starlink_staged_mailbox_control.py',
                      ...
                      'test_starlink_private_kernel_ordinal.py','test_starlink_ordinal_audit.py'}:
    ...
    if campaign in {'handover','admission','completion','replay','capture','ordinal'}:
    ...
    if campaign in {'admission','completion','replay','capture','ordinal'}:
        add('current/docs/starlink-staged-admission-20260911.md',ROOT/'docs/starlink-staged-admission-20260911.md')
    if campaign in {'completion','replay','capture','ordinal'}:
    ...
    if campaign in {'capture','ordinal'}:
    ...
    if campaign=='ordinal':
        add('current/docs/starlink-private-kernel-ordinal-20260911.md',ROOT/'docs/starlink-private-kernel-ordinal-20260911.md')
        add('reference/original_kernel_rom.v',RECOVERY/'staged-completion-prepared-v1/starlink_pss_kernel_rom.v')
        add('reference/capture_route_audit.json',RECOVERY/'staged-capture-route-v1/audit.json')
...
if __name__=='__main__':
    ...
    parser.add_argument('--campaign',choices=['output','handover','admission','completion','replay','capture','ordinal'],default='output')
