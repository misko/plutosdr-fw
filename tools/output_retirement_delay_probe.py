...
def prepare(path):
    ...
    bench=bench.replace('    run_buffered_auxiliary;','    output_retirement_boundary(0);',1)
...
'''
    ...
    return {'prepared':str(path),'sha256sums':pin,'qualification':False}
...
if __name__=='__main__':
    ...
    print(json.dumps(prepare(args.path) if args.command=='prepare' else actual.run('sim',args.path,args.pin,args.output),indent=2))
