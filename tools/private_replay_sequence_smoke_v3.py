...
import shutil
import private_replay_sequence_experiment_v3 as experiment
...
def prepare(path):
    ...
    bench=bench.replace('    run_buffered_auxiliary;','    auxiliary_active=1;aux_reset_boundary(2,0);aux_reset_boundary(2,1);aux_reset_boundary(3,0);aux_reset_boundary(3,1);aux_delay(1);run_private_replay_sequence_boundaries;',1)
...
def run(prepared,pin,output):
    ...
    result['private_replay_protocol']=experiment.witness((output/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text(),True)
