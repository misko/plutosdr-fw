...
def run(baseline,output):
    ...
    paths += [root/'tests'/name for name in ['test_starlink_guard_pulse.py','test_starlink_guard_pulse_mirror.py']]
    ...
    receipt=dict(command=command,baseline=str(baseline),baseline_sha256=before_sha,
        test_sources=hashes,started=time.time(),scope='133 inherited modules plus 2 guard-pulse modules; not entire repository')
    ...
    receipt['passed']=receipt['returncode']==0 and receipt['tests']==2390 and receipt['sources_unchanged'] and not any(list(result.iter(x)) for x in ['failure','error','skipped'])
