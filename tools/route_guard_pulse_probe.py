...
def run(sim,synth,tests,output):
    a=json.loads((sim/'guard_pulse_outcome.json').read_text())
    s=json.loads((synth/'guard_pulse_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['guard_pulse'],'actual guard and effective-state equivalence')
    tree=ET.parse(tests)
    require(len(list(tree.iter('testcase')))>=23 and not any(list(tree.iter(x)) for x in ['failure','error','skipped']),'component checks pass')
    ...
    inputs=[sim/'guard_pulse_outcome.json',synth/'guard_pulse_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
