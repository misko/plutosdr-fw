...
def run(sim,synth,tests,output):
    a=json.loads((sim/'ownership_lock_outcome.json').read_text())
    s=json.loads((synth/'ownership_lock_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['ownership_lock'],'actual ownership lock equivalence')
    tree=ET.parse(tests)
    require(len(list(tree.iter('testcase')))>=25 and not any(list(tree.iter(x)) for x in ['failure','error','skipped']),'component checks pass')
    ...
    inputs=[sim/'ownership_lock_outcome.json',synth/'ownership_lock_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
