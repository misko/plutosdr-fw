...
def run(route,output):
    ...
    tcl=Path(__file__).with_name('probe_private_forward_descriptor.tcl')
    ...
    try:
        ...
        require('PRIVATE_DESCRIPTOR_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        for label in ['descriptor_ce','descriptor_data','occupancy','identity']:
