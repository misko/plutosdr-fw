...
def run(route,output):
    ...
    tcl=Path(__file__).with_name('probe_product_retirement_receipt.tcl')
    ...
    try:
        ...
        require('PRODUCT_RETIREMENT_ENDPOINTS_OBSERVED_NOT_FULL_SIGNOFF' in raw and 'ERROR:' not in raw,'completed observation')
        report=(output/'reports/endpoints.txt').read_text()
        measured={}
        for label in ['occupancy','identity']:
        ...
        result.update(passed=True,endpoint_slack_ns=measured)
