...
static int visit_arguments(int argc,char **argv,struct visit_context *v,uint64_t lo[4])
{
    ...
    if(!strcmp(argv[argc-1],"1536-selected-observer3-scan64")) {
        ...
        v->followup_profile=v->fresh_continuity ? FRESH_SEGMENT30_PROFILE :
...
static int visit_status(const char *path,uint32_t rate,const char *profile)
{
    ...
    if(!strcmp(profile,SPARSE_PROFILE) || !strcmp(profile,SPARSE30_PROFILE) ||
       ...
       !strcmp(profile,FRESH_SEGMENT30_PROFILE)) {
        ...
        uint32_t maximum_attempts=!strcmp(profile,FRESH_SEGMENT30_PROFILE) ? 16U : segment ? 64U : 256U;
...
int main(int argc,char **argv)
{
    ...
    printf("{\"scope\":\"%s\",\"rate\":%u,\"result\":%d,\"rf_sample_limit\":%u",
        ...
        context.rate,rc,context.continuity ?
            (visits_executed-segments_started)*SCOUT_RF_SAMPLES+segments_started*SEGMENT_RF_SAMPLES :
            context.visit_count*SCOUT_RF_SAMPLES+(followup_started ? 737280000U : 0U));
