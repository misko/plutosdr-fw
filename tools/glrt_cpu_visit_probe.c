...
#define QUICK_SCOUT_PROFILE "1536-selected-observer3-scan64-scout1"
...
#define WAIT20X2_CONFIRM_100_PLAN "sparse100-wait20x2-confirm2-after-scout1"
...
#define WAIT100_PLAN_NS UINT64_C(1200000000000)
#define WAIT40_100_PLAN_NS UINT64_C(1800000000000)
#define WAIT40_CONTINUITY_ROUNDS 40U
#define SCOUT_RF_SAMPLES 25165824U
#define SEGMENT_RF_SAMPLES 122880000U
#define FOLLOWUP_RF_SAMPLES 737280000U
...
static int visit_arguments(int argc,char **argv,struct visit_context *v,uint64_t lo[4])
{
    ...
    if(!strcmp(argv[argc-1],"1536-selected-observer3-scan64")) {
        v->profile=argv[argc-1];count--;
              ...
              !strcmp(argv[argc-1],WAIT20X2_CONFIRM_100_PLAN)) {
        ...
        v->continuity=!strcmp(v->followup_plan,CONTINUITY30_PLAN) ||
            ...
            !strcmp(v->followup_plan,WAIT20X2_CONFIRM_100_PLAN);
        ...
        v->wait40x2=!strcmp(v->followup_plan,WAIT40X2_100_PLAN) ||
            !strcmp(v->followup_plan,WAIT40X2_CONFIRM_100_PLAN);
        v->wait20x2=!strcmp(v->followup_plan,WAIT20X2_CONFIRM_100_PLAN);
        v->persistent100=!strcmp(v->followup_plan,WAIT40X2_CONFIRM_100_PLAN) || v->wait20x2;
        v->continuity_rounds=v->wait20x2 ? 20U : v->wait40 ? WAIT40_CONTINUITY_ROUNDS :
            (v->wait_continuity || v->wait100) ? WAIT_CONTINUITY_ROUNDS : CONTINUITY_ROUNDS;
        v->continuity_segments=(v->wait40x2 || v->wait20x2) ? 2U : v->wait100 ? 1U :
            v->wait_continuity ? WAIT_CONTINUITY_SEGMENTS : CONTINUITY_ROUNDS;
        v->plan_ns=(v->wait40 || v->wait20x2) ? WAIT40_100_PLAN_NS :
...
static int visit_status(const char *path,uint32_t rate,const char *profile)
{
    ...
    if(status_number(raw,"rate",&actual_rate) || status_number(raw,"status",&status) ||
       ...
       status_number(raw,"worker_complete",&worker) || actual_rate!=rate || status || worker>1) return -1;
    if(!strcmp(profile,SCOUT_PROFILE) || !strcmp(profile,QUICK_SCOUT_PROFILE)) {
        uint32_t maximum_attempts=!strcmp(profile,QUICK_SCOUT_PROFILE) ? 1U : ATTEMPTS;
        if(blocks>BLOCKS || attempts>maximum_attempts || completed>1 || !worker) return -1;
    ...
    if(!strcmp(profile,SPARSE_PROFILE) || !strcmp(profile,SPARSE30_PROFILE) ||
       ...
       !strcmp(profile,FRESH_SEGMENT30_PROFILE) || !strcmp(profile,PRIOR_SEGMENT30_PROFILE)) {
        ...
        uint32_t maximum_attempts=(!strcmp(profile,FRESH_SEGMENT30_PROFILE) ||
...
struct continuity_result {
    unsigned selected,visits_executed,scan_rounds,segments_started;
    int track_complete,selected_activity;
};
    ...
    *result=(struct continuity_result){.selected=UINT_MAX};
    for(unsigned round=0;round<context->continuity_rounds && !result->track_complete &&
        ...
        if(rc==GLRT_VISIT_SIGNAL) {
            ...
            if(fprintf(context->journal,"segment start %u %u %u %" PRIu64 " %s\n",
               round,result->visits_executed,result->selected,lo[result->selected],
               context->activity_selected ? "retained_activity" : "native_handoff")<0 ||
               fflush(context->journal)) return GLRT_VISIT_RETENTION;
            context->profile=context->followup_profile;
            rc=glrt_tracking_visit_followup_run(ports,context->rate,lo[result->selected],
                result->visits_executed++);
            if(fprintf(context->journal,"segment terminal %u %d\n",round,rc)<0 ||
               fflush(context->journal)) return GLRT_VISIT_RETENTION;
...
int main(int argc,char **argv)
{
    ...
    unsigned selected=UINT_MAX,visits_executed=0,scan_rounds=0,segments_started=0;
    int followup_started=0,track_complete=0,selected_activity=0;
    struct continuity_result continuity={0};
    ...
    if(!rc && context.continuity) {
        ...
        selected=continuity.selected;visits_executed=continuity.visits_executed;
        scan_rounds=continuity.scan_rounds;segments_started=continuity.segments_started;
        track_complete=continuity.track_complete;selected_activity=continuity.selected_activity;
        followup_started=segments_started!=0;
    } else if(!rc && context.followup_sparse) {
        ...
        if(rc==GLRT_VISIT_SIGNAL) {
            ...
            track_complete=rc==GLRT_VISIT_DONE;
        }
    ...
    printf("{\"scope\":\"%s\",\"rate\":%u,\"result\":%d,\"rf_sample_limit\":%u",
        context.wait100 ? "bounded_arm_scout_wait100_followup" :
        ...
        context.rate,rc,context.continuity ?
            (visits_executed-segments_started)*SCOUT_RF_SAMPLES+segments_started*context.segment_rf_samples :
            context.visit_count*SCOUT_RF_SAMPLES+(followup_started ? FOLLOWUP_RF_SAMPLES : 0U));
    if(context.continuity) {
        printf(",\"scan_rounds\":%u,\"segments_started\":%u,\"visits_executed\":%u,\"track_complete\":%d,"
        ...
        if(context.ranked_continuity) printf(",\"activity_selection\":\"%s\"",
            context.wait20x2 ? "strongest_one_attempt_scan_power50_consecutive2_wait20x2_track100" :
