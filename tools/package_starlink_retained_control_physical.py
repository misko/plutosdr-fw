...
def main():
    ...
    roots = {
        'synthesis': recovery / 'retained-control-synth-parent.9xhaMMns',
        'route': recovery / 'retained-control-route-parent.rza0KS8U',
        'prepared_v1': recovery / 'retained-control-synthesis-prepared-v1',
    }
    before = {name: fixed_inventory(name, root) for name, root in roots.items()}
    synth = roots['synthesis'] / 'synthesis'
    route = roots['route'] / 'route'
    expected = {
        synth / 'retained_output_synth.dcp': '193cf14408ca768d403f1ef4bba913c07bf24a94bb88fb647941b95a03dd1482',
        route / 'retained_output_routed.dcp': '2ec838aa9a0a6f5ee297e506a07a842a1aa597f0e08241dc5bbf93b9493c1f3f',
        roots['prepared_v1'] / 'SHA256SUMS': '3590d4db36dd313a1db55419f46f1897bf8b588756e8b406f55537b1d7c135ee',
        route / 'probe.tcl': '034d1eaa197757b762644acd0cad9dc267338ae23fd5d757290c8485d0f1ac9a',
    }
    for path, digest in expected.items():
        assert receipt(path)['sha256'] == digest
    for line in (roots['prepared_v1'] / 'SHA256SUMS').read_text().splitlines():
        digest, name = line.split(maxsplit=1)
        assert receipt(roots['prepared_v1'] / name)['sha256'] == digest
    manifest = json.loads((synth / 'inputs/manifest.json').read_text())
    for name, value in manifest['files'].items():
        assert receipt(synth / 'inputs' / name) == value
        assert receipt(roots['prepared_v1'] / 'qualified' / name) == value
    ...
    output.mkdir()
    archive = output / 'retained-control-physical-v1.tar.gz'
