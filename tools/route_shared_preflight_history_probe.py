...
import shared_preflight_history_experiment as experiment
from staged_fft_experiment import sha,verify,require,fresh

def run(sim,synth,tests,output):
    a=json.loads((sim/'shared_preflight_outcome.json').read_text())
    s=json.loads((synth/'shared_preflight_outcome.json').read_text())
    ...
    require(experiment.witness((sim/'project/staged_fft.sim/sim_1/behav/xsim/simulate.log').read_text())==a['shared_preflight_history'],'actual word conservation')
    tree=ET.parse(tests)
    require(len(list(tree.iter('testcase')))>=6 and not any(list(tree.iter(x)) for x in ['failure','error','skipped']),'component checks pass')
    ...
    inputs=[sim/'shared_preflight_outcome.json',synth/'shared_preflight_outcome.json',tests,dcp,runner,Path(__file__).resolve()]
