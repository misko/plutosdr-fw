"""Archive source-matched local completion facts and physical observations."""
import argparse
import json
from pathlib import Path
import xml.etree.ElementTree as ET
from staged_fft_experiment import require, sha
from route_local_completion import evidence
...
def record(root):
    checked=evidence(root/'actual-v1',root/'synth-v1',root/'aux-actual-v2')
    total=tests(root/'regression-v1.xml',1165)+tests(root/'evidence-tests-v1.xml',26)
    ...
    for folder in ['prepared-v1','aux-prepared-v1','aux-prepared-v2']:
        for p in sorted((root/folder).iterdir()):
            if p.is_file():add(folder+'/'+p.name,p)
    for folder in ['actual-v1','aux-actual-v1','aux-actual-v2','synth-v1','route-v1']:
        base=root/folder
    ...
    for folder in ['regression-v1','evidence-tests-v1','component-v1']:
    ...
    for pattern in ['tools/*local_completion*.py','tests/test_starlink_local_completion*.py','docs/starlink-local-completion-*.md']:
    ...
    return write_verified_archive(root/'20260911-local-completion-facts-evidence.tgz',sources)
