...
output_dir="${1:-${repository_root}/build/starlink-pss15-iq-to-phase-map-xfft-ooc}"
...
(
  ...
  LD_LIBRARY_PATH="$compat_lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    "$vivado_bin" -mode batch -nojournal -nolog \
      -source "$acquisition_dir/synthesize_iq_to_phase_map_xfft_ooc.tcl" \
      -tclargs "$output_dir" 2>&1 | tee iq_to_phase_map_xfft_ooc_transcript.log
)
