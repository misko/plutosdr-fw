...
set -euo pipefail
if [[ $# -lt 2 || $# -gt 3 ]]; then
  echo 'usage: build_glrt_core_ooc.sh RATE_HZ NEW_OUTPUT_DIRECTORY [ddc|correlator]' >&2
  exit 2
fi
case "$1" in 2500000|5000000|10000000|25000000|60000000) ;; *) exit 2 ;; esac
core=${3:-ddc}
case "$core" in ddc|correlator) ;; *) exit 2 ;; esac
...
arguments=("$1" "$output")
if [[ "$core" == correlator ]]; then arguments+=(correlator); fi
