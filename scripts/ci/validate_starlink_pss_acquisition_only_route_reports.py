#!/usr/bin/env python3
"""Validate the exact routed CDC inventory of acquisition-only PSS RX."""
...
EXPECTED_BUS_SKEW_ENDPOINTS = (8, 8, 32, 4, 4)
...
class ValidationError(RuntimeError):
    """A routed report differs from the reviewed acquisition-only inventory."""
...
def validate_cdc_report(text: str) -> None:
    if "starlink_pss_tracker" in text:
        raise ValidationError("acquisition-only CDC report contains tracker hierarchy")
    ...
    if summary != EXPECTED_SUMMARY or len(summary_rows) != len(EXPECTED_SUMMARY):
        raise ValidationError(
            f"acquisition-only CDC summary differs: expected {EXPECTED_SUMMARY}, got {summary}"
        )
    rows = [match.group(0) for match in DETAIL_ROW.finditer(text)]
    expected_rows = sum(EXPECTED_SUMMARY.values())
    if len(rows) != expected_rows:
        raise ValidationError(
            f"acquisition-only CDC detail count differs: expected {expected_rows}, got {len(rows)}"
        )
    ...
    for rule, source, destination in CRITICAL_CROSSINGS:
        ...
        if len(matches) != 1:
            raise ValidationError(f"reviewed critical crossing missing or duplicated: {source}")
    multibit_rows = [
    ...
    for source, destination in MULTIBIT_CROSSINGS:
        ...
        if len(matches) != 1:
            raise ValidationError(f"reviewed multibit crossing missing or duplicated: {source}")


def validate_bus_skew_report(text: str) -> None:
    if "starlink_pss_tracker" in text:
        raise ValidationError("acquisition-only bus-skew report contains tracker hierarchy")
    if "Slack (VIOLATED)" in text:
        raise ValidationError("acquisition-only bus-skew report has a violation")
    expected_count = len(EXPECTED_BUS_SKEW_ENDPOINTS)
    met = text.count("Slack (MET)")
    if met != expected_count:
        raise ValidationError(f"expected exactly {expected_count} met constraints, got {met}")
    ...
    if endpoint_counts != EXPECTED_BUS_SKEW_ENDPOINTS:
        raise ValidationError(
            "acquisition-only bus-skew endpoint inventory differs: "
    ...
    for source in BUS_SKEW_SOURCES:
        ...
        if len(matches) != 1:
            raise ValidationError(f"reviewed bus-skew source missing or duplicated: {source}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("PASS exact acquisition-only PSS RX routed CDC and five-constraint bus-skew inventories")
    return 0
