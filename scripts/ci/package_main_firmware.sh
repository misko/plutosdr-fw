...
adaptive-scan-v2-source.yaml | adaptive-scan-v1-source.yaml | iq-direct-async-v5-source.yaml | counter-rx-v1-source.yaml | iq-direct-async-v4-source.yaml | iq-direct-async-v3-source.yaml | iq-direct-async-v2-source.yaml | iq-direct-async-ring-v1-rc1-source.yaml | iio-throughput-coverage-window-v6-rc1-source.yaml | \
...
if [[ "$(basename "$MANIFEST")" == adaptive-scan-v2-source.yaml ]]; then
    adaptive_dtbs=()
    for fdt_index in 0 1 2; do
        packed_dtb="$ARTIFACT_ROOT/packed-fdt-${fdt_index}.dtb"
        dumpimage -T flat_dt -p "$fdt_index" -o "$packed_dtb" "$dfu"
