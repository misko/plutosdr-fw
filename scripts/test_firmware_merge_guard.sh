...
git -C "$repository" switch -q --detach "$denied_sha"
git -C "$repository" switch -q -c fixture-denied-lineage
printf 'follow-up\n' >"$repository/rtl/follow-up.v"
lineage_sha=$(git_commit 'continue denied lineage')
expect_reject 'denylisted ancestry' 'descends from denied experimental lineage' \
  codex/renamed-experiment "$base_sha" "$lineage_sha"
...
for protected_path in \
  ...
  expect_reject "modified $protected_path" 'changes protected base policy' \
...
expect_reject 'modify then revert guard' 'history changes protected base policy' \
...
expect_reject 'protected path in merged unrelated root' \
  'history changes protected base policy' \
...
expect_reject 'deleted guard' 'changes protected base policy' \
