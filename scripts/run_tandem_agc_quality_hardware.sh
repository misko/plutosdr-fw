...
MANIFEST=${IIO_MANIFEST:-${ROOT}/manifests/tandem-agc-v8-rc4-source.yaml}
...
IIO_BUILD=${IIO_BUILD:-${IIO_SOURCE}/build-tandem-quality-${expected_commit:0:12}}
...
printf '%s\n' \
  'INFO: this runner does not deploy or flash firmware.' \
  "INFO: the opened radio's exact --firmware-pattern match is the deployment attestation."
